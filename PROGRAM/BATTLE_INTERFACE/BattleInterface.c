...
void BI_SetPossibleCommands()
{
	...
	if(mainIdx==chIdx)
	{
		...
		if( !bEnableIslandSailTo && !bArcadeSailing ) {
...
ref procGetSailTextureData()
{
	...
	if(chrIdx>=0)
	{
		...
		if(st!=SHIP_NOTUSED)
		{
			...
			if( CheckAttribute(&Characters[chrIdx],"Features.GeraldSails") && sti(Characters[chrIdx].Features.GeraldSails)==true && shref.Name != "BlackPearl") 
			{
...
void BI_ProcessControlPress()
{
	...
	case "hk_charge1":
		PlaySound("interface\_balls.wav");
		Ship_ChangeCharge(chr, GOOD_BALLS);
	break;

	case "hk_charge2":
		PlaySound("interface\_grapes.wav");
		Ship_ChangeCharge(chr, GOOD_GRAPES);
	break;

	case "hk_charge3":
		PlaySound("interface\_chain.wav");
		Ship_ChangeCharge(chr, GOOD_KNIPPELS);
	break;

	case "hk_charge4":
		PlaySound("interface\_bombs.wav");
		Ship_ChangeCharge(chr, GOOD_BOMBS);
	break;

	case "hk_ImmediateReload":
		if( GetCharacterPerkUsing(chr,"ImmediateReload") )
		{
			ActivateCharacterPerk(chr,"ImmediateReload");
		}
	break;

	case "hk_LightRepair":
		if( GetCharacterPerkUsing(chr,"LightRepair") )
		{
			ActivateCharacterPerk(chr,"LightRepair");
			ActivateSpecRepair(chr,0);
		}
	break;

	case "hk_InstantRepair":
		if( GetCharacterPerkUsing(chr,"InstantRepair") )
		{
			ActivateCharacterPerk(chr,"InstantRepair");
			ActivateSpecRepair(chr,1);
		}
	break;

	case "hk_Turn180":
		if( GetCharacterPerkUsing(chr,"Turn180") )
		{
			ActivateCharacterPerk(chr,"Turn180");
			Ship_Turn180(chr);
		}
	break;

	case "hk_SandbankManeuver":
		if( GetCharacterPerkUsing(chr,"SandbankManeuver") )
		{
			ActivateCharacterPerk(chr,"SandbankManeuver");
		}
	break;
	}
}
...
float GetRSRollSpeed(ref chref)
{
	...
	int crewMax = GetMaxCrewQuantity(chref);
	if(crewQ<crewMin) fRollSpeed *= makefloat(crewQ)/makefloat(crewMin);
	fRollSpeed *= makefloat(crewQ)/makefloat(crewMax);
	if (!bArcadeSailing)
	{
		fRollSpeed = fRollSpeed / 2.0;
	}
	return fRollSpeed;
}
