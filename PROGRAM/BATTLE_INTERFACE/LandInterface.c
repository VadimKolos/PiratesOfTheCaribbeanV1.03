...
void BLI_ExecuteCommand()
{
	...
	break;
	case "BI_ActivateShotGun":
		globalSGMode = !globalSGMode;
		if(globalSGMode) Log_SetStringToLog("Shotgun mode ON");
		else Log_SetStringToLog("Shotgun mode OFF");
...
void BLI_SetObjectData()
{
	...
	objLandInterface.CommandTextures.list.t6.name = "battle_interface\LandCommands2.tga";
	objLandInterface.CommandTextures.list.t6.xsize = 1;
	objLandInterface.CommandTextures.list.t6.ysize = 1;
	...
	objLandInterface.Commands.ActivateShotgun.texNum = 6;
	objLandInterface.Commands.ActivateShotgun.enable	= false;
	objLandInterface.Commands.ActivateShotgun.picNum	= 0;
	objLandInterface.Commands.ActivateShotgun.event	= "BI_ActivateShotgun";
	objLandInterface.Commands.ActivateShotgun.note		= LanguageConvertString(idLngFile, "land_ActivateShotgun");
...
void BLI_SetPossibleCommands()
{
	...
	if(CheckCharacterItem(mchref,"Shotgun"))
	{
		objLandInterface.Commands.ActivateShotgun.enable = true;
		bUseCommand = true;
	}
...
void LI_ProcessControlPress()
{
	...
	{
	case "hk_Cure":
		if( LAi_GetCharacterRelHP(chr) < 1.0 )
		{
			float dhlt = LAi_GetCharacterMaxHP(chr) - LAi_GetCharacterHP(chr);
			string btl = FindHealthForCharacter(chr, dhlt);
			DoCharacterUsedItem(chr, btl);
		}
	break;

	case "hk_Rush":
		if( GetCharacterPerkUsing(chr,"Rush") )
		{
	...
	case "hk_Shotgun":
		if( GetCharacterPerkUsing(chr,"Shotgun") )
		{
			ActivateCharacterPerk(chr,"Shotgun");
		}
	break;
	}
}
