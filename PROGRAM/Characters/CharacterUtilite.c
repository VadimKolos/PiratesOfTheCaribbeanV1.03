...
int AddCharacterGoods(ref _refCharacter,int _Goods,int _Quantity)
{
	...
	for(i=0; i<4; i++)
	{
		...
		if(cn!=-1)
		{
			...
			if( GetRemovable(chref) )
			{
				freeQuantity = GetGoodQuantityByWeight( _Goods, GetCargoFreeSpace(chref) );
				if(freeQuantity>=_Quantity)
				{
					chref.Ship.Cargo.Goods.(goodsName) = sti(chref.Ship.Cargo.Goods.(goodsName)) + _Quantity;
					RecalculateCargoLoad(chref);
					return true;
				}
				chref.Ship.Cargo.Goods.(goodsName) = sti(chref.Ship.Cargo.Goods.(goodsName)) + freeQuantity;
...
int RemoveCharacterGoods(ref _refCharacter,int _Goods,int _Quantity)
{
	...
	for(i=0; i<4; i++)
	{
		...
		if(cn!=-1)
		{
			...
			if( GetRemovable(chref) )
			{
				curQuantity = sti( chref.Ship.Cargo.Goods.(goodsName) );
				if(curQuantity>=_Quantity)
				{
					chref.Ship.Cargo.Goods.(goodsName) = curQuantity - _Quantity;
					RecalculateCargoLoad(chref);
					return true;
				}
				chref.Ship.Cargo.Goods.(goodsName) = 0;
...
int AddCharacterExp(ref _refCharacter,int _exp)
{
	...
	if( CheckCharacterPerk(_refCharacter,"SharedExperience") )
	{
		...
		for(i=0; i<PASSENGERS_MAX; i++)
		{
			cn = GetPassenger(_refCharacter,i);
			if( cn<0 ) break;
			AddCharacterExp(GetCharacter(cn),(_exp+6)/7);
		}
		for(i=1; i<4; i++)
		{
			cn = GetOfficersIndex(_refCharacter,i);
...
{
	if (characters[0].paymenttype == "percent" && characters[0].repair == "0" && _money >0)
	{
		_refCharacter.month.money = makeint(_refCharacter.month.money) + _Money;
	}
	//уменьшаме деньги за починку судов
	if (characters[0].paymenttype == "percent" && characters[0].repair == "1" && _money <0)
	{
		_refCharacter.month.money = makeint(_refCharacter.month.money) + _Money;
	}
	characters[0].repair = "0";
