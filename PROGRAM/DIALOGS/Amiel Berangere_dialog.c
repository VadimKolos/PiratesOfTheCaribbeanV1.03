//nclude "DIALOGS\Amiel Berangere_dialog.h"
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "First time":
			...
			d.Text = DLG_TEXT[0] + Address_Form.fra + DLG_TEXT[1];
			Link.l1 = PChar.name + DLG_TEXT[2];
			Link.l1.go = "Node_1";
		break;

		case "Node_1":
			dialog.snd = "Voice\AMBE\AMBE002";
			d.Text = NPChar.name + DLG_TEXT[3] + PChar.name + DLG_TEXT[4];
			Link.l1 = DLG_TEXT[5];
			Link.l1.go = "Node_2";
		break;
		...
		case "Node_8":
			if(makeint(Pchar.skill.leadership) >= 5)
			{
				...
				Link.l1 = DLG_TEXT[24];
				Link.l1.go = "Node_11";
				if(makeint(Pchar.skill.leadership) >= 8)
				{
					Link.l2 = DLG_TEXT[25];
		...
		case "Node_10":
			...
			d.Text = DLG_TEXT[32] + PChar.name + DLG_TEXT[33];
			Link.l1 = DLG_TEXT[34];
			Link.l1.go = "Node_4";
			Link.l2 = DLG_TEXT[35];
			Link.l2.go = "Node_6";
		...
		case "Node_12":
			...
			d.Text = DLG_TEXT[44];
			Link.l1 = DLG_TEXT[45] + NPChar.name + DLG_TEXT[46];
			Link.l1.go = "Exit";
		break;
		...
		case "Node_work_2":
			...
			if(makeint(PChar.money) >= 10000)
			{
				...
				Link.l2 = DLG_TEXT[56] + NPChar.name + DLG_TEXT[57];
				Link.l2.go = "Node_10";
			}
			else
			{
		...
		case "Node_work_3":
			...
			d.Text = DLG_TEXT[59] + PChar.name + DLG_TEXT[60];
			Link.l1 = DLG_TEXT[61];
		...
		case "Node_work_4":
			...
			Link.l1 = DLG_TEXT[66] + NPChar.name + DLG_TEXT[67];
			Link.l1.go = "exit2";
		...
		case "Second time":
			...
			d.Text = DLG_TEXT[68] + Pchar.name + DLG_TEXT[69];
			Link.l1 = DLG_TEXT[70];
