//nclude "DIALOGS\Arabella Silehard_dialog.h"
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "Find_Lucas1":
			...
			link.l1 = Characters[GetCharacterIndex(DLG_TEXT[9])].name + DLG_TEXT[10];
			link.l1.go = "Find_Lucas2";
			link.l2 = DLG_TEXT[11] + Characters[GetCharacterIndex(DLG_TEXT[12])].name + DLG_TEXT[13] + Characters[GetCharacterIndex(DLG_TEXT[14])].name + DLG_TEXT[15];
			link.l2.go = "Find_Lucas3";
		break;

		case "Find_Lucas2":
			dialog.snd = "Voice\ARSI\ARSI004";
			dialog.text = DLG_TEXT[16] + Characters[GetCharacterIndex(DLG_TEXT[17])].name + DLG_TEXT[18];
		...
		case "Find_Lucas3":
			...
			dialog.text = DLG_TEXT[22] + Characters[GetCharacterIndex(DLG_TEXT[23])].name + DLG_TEXT[24];
			link.l1 = DLG_TEXT[25];
			link.l1.go = "Find_Lucas_exit";
		break;
