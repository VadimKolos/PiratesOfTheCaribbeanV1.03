//nclude "DIALOGS\Everard Gordon_dialog.h"
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "First time":
			...
			else
			{
				...
				d.Text = RandPhrase(DLG_TEXT[4] + PChar.name + DLG_TEXT[5], TimeGreeting() + DLG_TEXT[6], DLG_TEXT[7] + address_form.eng + " " + PChar.name + DLG_TEXT[8], &dialog, dialog.snd1, dialog.snd2, dialog.snd3);
				Link.l1 = DLG_TEXT[9];
				...
				link.l3.go = "room";
				Link.l4 = DLG_TEXT[13] + NPChar.name + DLG_TEXT[14];
				Link.l4.go = "exit";
			} 
		break;
		...
		case "Meeting":
			...
			d.Text = DLG_TEXT[15] + PChar.name + DLG_TEXT[16], ;
			Link.l1 = DLG_TEXT[17];
			if(CheckAttribute(PChar, "Quest.Story_OxbayCaptured"))
			{
				Link.l1.go = "no crew";
			...
			Link.l2 = NPChar.name + DLG_TEXT[18];
			Link.l2.go = "quest lines";
			link.l3 = pcharrepphrase(DLG_TEXT[19], DLG_TEXT[20]);
			link.l3.go = "room";
			Link.l4 = DLG_TEXT[21] + NPChar.name + DLG_TEXT[22];
			Link.l4.go = "exit";
		break;
		...
		case "quest lines":
			...
			d.Text = DLG_TEXT[36] + PChar.name + DLG_TEXT[37];
		...
		case "quest lines_1":
			...
			{
				Link.l1 = DLG_TEXT[43] + Characters[GetCharacterIndex(DLG_TEXT[44])].name + DLG_TEXT[45];
				...
				else
				{
					...
					else
					{
						...
						if (randomGod == 1)
						{
							...
							link.l1 = DLG_TEXT[59];
							link.l1.go = "exit";
							pchar.quest.destination = "Oxbay_port";
		...
		case "rumours":
			...
			Link.l2 = DLG_TEXT[65];
			Link.l2.go = "quest lines";
			Link.l3 = DLG_TEXT[66] + NPChar.name + DLG_TEXT[67];
			Link.l3.go = "exit";
		break;
		...
		case "no quest":
			...
			Link.l2 = DLG_TEXT[70];
			Link.l2.go = "quest lines";
			Link.l3 = DLG_TEXT[71] + NPChar.name + DLG_TEXT[72];
			Link.l3.go = "exit";
		break;
		...
		case "no crew":
			...
			Link.l2 = DLG_TEXT[75] + NPChar.name + DLG_TEXT[76];
			Link.l2.go = "exit";
		break;
		...
		case "Story_1stTask":
			...
			if(Pchar.id == "Blaze")
			{
				...
				d.Text = DLG_TEXT[77] + characters[GetCharacterIndex(DLG_TEXT[78])].name + DLG_TEXT[79];
				Link.l1 = DLG_TEXT[80] + NPChar.name + DLG_TEXT[81];
				Link.l1.go = "Story_1stTask_Male_1";
			}
			else
			{
				dialog.snd = "Voice\EVGO\EVGO015";
				d.Text = DLG_TEXT[82];
				Link.l1 = DLG_TEXT[83];
				Link.l1.go = "Story_1stTask_Female_1";
			}
		break;
		...
		case "Story_1stTask_Male_4":
			...
			d.Text = DLG_TEXT[90] + PChar.name + DLG_TEXT[91];
			Link.l1 = DLG_TEXT[92];
		...
		case "Story_1stTask_Male_10":
			...
			d.Text = DLG_TEXT[104] + PChar.name + DLG_TEXT[105];
		...
		case "Story_1stTask_Male_11":
			...
			d.Text = DLG_TEXT[107] + PChar.name + DLG_TEXT[108];
		...
		case "Story_1stTask_Male_12":
			if(makeint(Pchar.skill.Leadership) >= 2)
			{
		...
		case "Story_1stTask_Male_13":
			...
			d.Text = DLG_TEXT[116] + (10*(10 - makeint(Pchar.skill.commerce))) + DLG_TEXT[117];
			if(makeint(Pchar.money) >= (10*(10 - makeint(Pchar.skill.commerce))))
			{
		...
		case "Story_1stTask_Male_15":
			...
			if(makeint(Pchar.money) >= (10*(10 - makeint(Pchar.skill.commerce))))
			{
				AddMoneyToCharacter(Pchar, -(10*(10 - makeint(Pchar.skill.commerce))));
				dialog.snd = "Voice\EVGO\EVGO031";	
				d.Text = DLG_TEXT[122] + PChar.name + DLG_TEXT[123];
		...
		case "AskAboutBeauvallet":
			...
			d.Text = Characters[GetCharacterIndex(DLG_TEXT[132])].name + " " + Characters[GetCharacterIndex(DLG_TEXT[133])].lastname + DLG_TEXT[134];
		...
		case "frigate_sink_start_3":	
			...
			{	
				...
				Link.l1 = DLG_TEXT[141] + NPchar.name + DLG_TEXT[142];
				Link.l1.go = "exit";
			}
		...
		case "frigate_sink_crew_1":
			...
			d.Text = DLG_TEXT[146] + address_form.eng + DLG_TEXT[147];
