//nclude "DIALOGS\Ewan Glover_dialog.h"
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "First time":
			...
			d.Text = DLG_TEXT[0] + Address_Form.eng + " " + PChar.name + DLG_TEXT[1] + Characters[GetCharacterIndex(DLG_TEXT[2])].name + " " + Characters[GetCharacterIndex(DLG_TEXT[3])].lastname + DLG_TEXT[4];
		...
		case "Node_1":
			...
			d.Text = DLG_TEXT[8];
			Link.l1 = DLG_TEXT[9] +  Characters[GetCharacterIndex(DLG_TEXT[10])].name + DLG_TEXT[11];
			Link.l1.go = "Node_2";
		break;
		...
		case "Node_3":
			if(SetCompanionIndex(Pchar, -1, GetCharacterIndex("Ewan Glover")) == GetCharacterIndex("Ewan Glover"))
			{
				...
				d.Text = DLG_TEXT[14] + PChar.name + DLG_TEXT[15];
				Link.l1 = DLG_TEXT[16];
				Link.l1.go = "Node_4";
			}
			else
		...
		case  "Node_4":
			...
			d.text = DLG_TEXT[19] + Characters[GetCharacterINdex(DLG_TEXT[20])].name + DLG_TEXT[21];
			Link.l1 = DLG_TEXT[22];
		...
		case "Second Time":
			...
			d.Text =  DLG_TEXT[23] + PChar.name + DLG_TEXT[24]; 
			Link.l1 = DLG_TEXT[25];
			Link.l1.go =  "exit";
		...
		case "Are_you_ready":
			...
			else
			{
				...
				d.Text = DLG_TEXT[36] + address_form.eng + " " + Pchar.name + DLG_TEXT[37];
				Link.l1 = DLG_TEXT[38];
		...
		case "Are_you_ready_again":
			...
			d.Text = DLG_TEXT[60] + Pchar.name + DLG_TEXT[61];
			Link.l1 = DLG_TEXT[62];
			Link.l1.go = "Exit";
		...
		case "Goodbye_1":
			...
			d.Text = DLG_TEXT[65] + Pchar.name + DLG_TEXT[66];
			Link.l1 = DLG_TEXT[67] + NPChar.name + DLG_TEXT[68];
