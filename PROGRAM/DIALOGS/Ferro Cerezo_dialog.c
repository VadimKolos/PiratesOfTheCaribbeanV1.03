//nclude "DIALOGS\Ferro Cerezo_dialog.h"
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "First time":
			...
			if (pchar.quest.main_line == "search_for_Ferro_Cerezo" || pchar.quest.main_line == "search_for_Ferro_Cerezo_find")
			{
				...
				Link.l1 = DLG_TEXT[3] + npchar.name + DLG_TEXT[4];
			...
			if ( CheckQuestAttribute("Story_DominguesCompromat","Find") )
			{
				...
				Link.l1 = DLG_TEXT[8] + npchar.name + DLG_TEXT[9];
		...
		case "Danielle_talk_2":
			...
			if (GetCharacterSKILL(pchar, SKILL_LEADERSHIP)>7 && sti(pchar.reputation)<30)
			{
				link.l1 = DLG_TEXT[15];
		...
		case "Danielle_buy_1":
			if (GetCharacterSkill(pchar, SKILL_COMMERCE) > 4)
			{
		...
		case "Danielle_buy_2":
			if (GetCharacterSkill(pchar, SKILL_COMMERCE) > 5)
			{
		...
		case "Danielle_buy_3":
			if (GetCharacterSkill(pchar, SKILL_COMMERCE) > 6)
			{
		...
		case "talk_in_tavern_2":
			...
			dialog.text = DLG_TEXT[49] + characters[getCharacterIndex(DLG_TEXT[50])].lastname + DLG_TEXT[51];
			link.l1 = DLG_TEXT[52];
		...
		case "talk_in_tavern_4":
			...
			if (GetCharacterSKILL(pchar, SKILL_LEADERSHIP) && makeint(pchar.reputation) < 30)
			{
				link.l1 = DLG_TEXT[56];
		...
		case "leadership":
			...
			link.l1 = DLG_TEXT[59] + pchar.name + " " + pchar.lastname + DLG_TEXT[60];
