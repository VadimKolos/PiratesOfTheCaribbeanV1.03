//nclude "DIALOGS\Friedrich Corleis_dialog.h"  //v 1.03
...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "quests":
			...
			{
				link.l3 = DLG_TEXT[18] + characters[GetCharacterIndex(DLG_TEXT[19])].name + " " + characters[GetCharacterIndex(DLG_TEXT[20])].lastname + DLG_TEXT[21];
				link.l3.go = "search_rheims";
				iTest = iTest + 1;
			}
			...
			if (CheckQuestAttribute("amanda_status", "goSearchAmanda"))
			{
				dialog.text = "Да, капитан, что вы хотели.";
				link.l3 = "Я ищу одну женщину, которая недавно на остров, её имя Аманда Медейруш. ";
				link.l3.go = "portugalLineSearchA";
			}
		...
		case "blaze_to_incas_collection_begin_2":
			...
			link.l1 = DLG_TEXT[51] + characters[getCharacterIndex(DLG_TEXT[52])].name + " " + characters[getCharacterIndex(DLG_TEXT[53])].lastname + DLG_TEXT[54];
				...
				else
				{
					...
					else
					{
						...
						if (randomGod == 1)
						{
							...
							link.l1 = DLG_TEXT[85];
							link.l1.go = "exit";
							pchar.quest.destination = "Douwesen_port";
		...
		case "portugalLineSearchA":
			dialog.snd = "Voice\FRCO\FRCO015";
			dialog.text = "Не думаю, что это имя мне о чем-то говорит, минхер.";
			link.l1 = "Это молода женщина, португалка, в ярком красном платье, я думаю она искала судно на которое могла бы сесть пассажиром.";
			link.l1.go = "portugalLineSearchA2";
			break;

		case "portugalLineSearchA2":
			dialog.text = "Кажется теперь припоминаю, да здесь была эта женщина, она искала человека, который бы мог сопроводить её до пиратского поселения на другом конце острова";
			link.l1 = "Ты видел как она туда отправилась?";
			link.l1.go = "portugalLineSearchA3";
			break;

		case "portugalLineSearchA3":
			dialog.text = "Нет, но я думаю в лагере пиратов вы сможете что-нибудь узнать.";
			link.l1 = "Что ж, тогда отправлюсь туда.";
			link.l1.go = "portugalLineSearchA4";
			break;

		case "portugalLineSearchA4":
			AddDialogExitQuest("AmandaInDouPirCamp");
			AddQuestRecord("PortuguezeLine", “7”);
			DialogExit();
			break;
	}
}
