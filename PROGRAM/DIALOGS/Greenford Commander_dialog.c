...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "dopros_2":
			...
			if (GetCharacterSkill(pchar, SKILL_LEADERSHIP) > 3 || CheckQuestAttribute("capture_greenford", "captured"))
			{
				link.l1 = DLG_TEXT[16];
				link.l1.go = "dopros_3";
			}
			if (!CheckQuestAttribute("capture_greenford", "captured"))
			{
				link.l2 = DLG_TEXT[17];
		...
		case "dopros_3":
			if (GetCharacterSkill(pchar, SKILL_LEADERSHIP) > 5 || CheckQuestAttribute("capture_greenford", "captured"))
			{
