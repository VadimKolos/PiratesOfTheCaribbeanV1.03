...
void ProcessDialogEvent()
{
	...
	case "First time":
		...
		else
		{
			...
			Link.l3.go = "exit";
			if (CheckQuestAttribute("portuguese_line", "searchInformation"))
			{
				link.l4 = "У меня к вам дело, сэр.";
	...
	case "portugalLineStart2":
		d.Text = "Я вас слушаю, капитан.";
		Link.l1 = "Скажите, вам знаком человек по имени Руи Феррара? Я занимаюсь его поисками по пручению губернатора Консейсао.";
		Link.l1.go = "portugalLineStart3";
		break;

	case "portugalLineStart3":
		d.Text = "Боюсь что да, капитан. Сеньор Феррара был моим деловым партнером...";
		Link.l1 = "Был?";
		Link.l1.go = "portugalLineStart4";
		break;

	case "portugalLineStart4":
		d.Text = "Да, капитан. Покойный сеньор Феррара прибыл по одному делу ко мне. Я принял Руи у себя дома, однако вскоре был вынужден покинуть его," +
			" я должен был встретиться с комендантом. По возвращению я нашел мертвым сеньора Феррару. Его зарезали.";
		Link.l1 = "Можете прояснить когда это было? На долго вы уходили?";
		Link.l1.go = "portugalLineStart5";
		break;


	case "portugalLineStart5":
		d.Text = "Покойный прибыл ко мне утром, всего я пробыл с ним около часа, а потом оставил его. Возможно мне даже повезло...";
		Link.l1 = "Я правильно понял, сеньора Феррару убили прямо посреди дня?";
		Link.l1.go = "portugalLineStart6";
		break;

	case "portugalLineStart6":
		d.Text = "Да, правда я не могу сказать когда именно, знаю только то, что это был настоящий головорез, здесь было море крови...";		
		Link.l1.go = "portugalLineStart7";
		break;

	case "portugalLineStart7":
		d.Text = "Я даже не смог смотреть на это зрелище и сразу позвал стражу.";
		Link.l1 = "Видели ли кого-нибудь, кто заходил к вам в дом после вашего ухода?";
		Link.l1.go = "portugalLineStart8";
		break;

	case "portugalLineStart8":
		d.Text = "Не знаю. Боюсь я был не в состоянии выяснять обстоятельства.";
		Link.l1 = "А вы еще говорили, что сеньор Феррара прибыл к вам по делу. Можно ли узнать в чем оно заключалось?";
		Link.l1.go = "portugalLineStart9";
		break;

	case "portugalLineStart9":
		d.Text = "Боюсь, что нет, капитан. Я не могу вам сказать, да и к тому же это вряд ли вам поможет в расследовании.";
		Link.l1 = "Но как я понял, вы заранее договаривались о встрече?";
		Link.l1.go = "portugalLineStart10";
		break;

	case "portugalLineStart10":
		d.Text = "Нет, мы договорились встретиться в Гринфорде, при любом удобном случае, когда сеньор Феррара будет рядом, он не предупреждал меня заранее.";
		Link.l1 = "Может вы помните, каким образом покойный добирался на Оксбэй?";
		Link.l1.go = "portugalLineStart11";
		break;

	case "portugalLineStart11":
		d.Text = "Насколько я помню он прибыл на судне с португальским флагом.";
		Link.l1 = "Что ж, пока что у меня больше нет вопросов.";
		Link.l1.go = "portugalLineStart12";
		break;

	case "portugalLineStart12":
		AddDialogExitQuest("FerraraWasKilled");
		AddQuestRecord("PortuguezeLine", “4”);
		DialogExit();
		break;
	}
}
