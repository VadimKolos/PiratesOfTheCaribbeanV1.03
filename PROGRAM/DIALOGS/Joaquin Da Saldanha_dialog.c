//nclude "DIALOGS\Joaquin Da Saldanha_dialog.h"
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "First time":
			...
			{
				dialog.text = DLG_TEXT[2] + Characters[GetCharacterIndex(DLG_TEXT[3])].name + " " + Characters[GetCharacterIndex(DLG_TEXT[4])].lastname + DLG_TEXT[5];
				link.l1 = DLG_TEXT[6];
		...
		case "Find_Lucas2":
			...
			dialog.text = DLG_TEXT[18] + Characters[GetCharacterIndex(DLG_TEXT[19])].name + DLG_TEXT[20];
			link.l1 = DLG_TEXT[21];
		...
		case "Find_Lucas4":
			...
			dialog.text = DLG_TEXT[24] + Characters[GetCharacterIndex(DLG_TEXT[25])].name + DLG_TEXT[26];
			link.l1 = DLG_TEXT[27];
			link.l1.go = "Find_Lucas5";
			link.l2 = DLG_TEXT[28];
		...
		case "Find_Lucas5":
			...
			dialog.text = DLG_TEXT[29] + Characters[GetCharacterIndex(DLG_TEXT[30])].name + DLG_TEXT[31];
			link.l1 = DLG_TEXT[32];
		...
		case "Ransom_Lucas1":
			...
			dialog.text = DLG_TEXT[35] + Characters[GetCharacterIndex(DLG_TEXT[36])].name + DLG_TEXT[37];
			link.l1 = DLG_TEXT[38];
		...
		case "Ransom_Lucas2":
			...
			dialog.text = DLG_TEXT[39] + Characters[GetCharacterIndex(DLG_TEXT[40])].name + DLG_TEXT[41];
			link.l1 = DLG_TEXT[42];
		...
		case "Ransom_Lucas3":
			...
			dialog.text = DLG_TEXT[44] + Characters[GetCharacterIndex(DLG_TEXT[45])].name + DLG_TEXT[46];
			link.l1 = DLG_TEXT[47];
