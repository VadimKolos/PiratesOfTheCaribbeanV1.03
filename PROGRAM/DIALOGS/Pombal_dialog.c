	...
	switch(Dialog.CurrentNode)
	{
		...
		case "First time":
			...
			else
			{
					...
					Link.l3.go = "exit";
					if (CheckQuestAttribute("portuguese_line", "goToBankir"))
					{
						link.l4 = "Губернатор Баррето поручил мне выяснить судьбу пропавшего сеньора Феррары.";
		...
		case "portugalLineStart2":
			d.Text = "Я готов ответить на все ваши вопросы, капитан.";
			Link.l1 = "Отлично, расскажите в подробностях обстоятельства исчезновения Руи Феррары.";
			Link.l1.go = "portugalLineStart3";
			break;

		case "portugalLineStart3":
			d.Text = "Я даже не знаю что я могу вам сказать по этому поводу. Я работаю помощником у сеньора Феррары, и в один прекрасный момент он просто перестал"
				+ " появляться здесь в собственном доме. Я попытался самостоятельно выяснить что происходит, но после нескольких дней безрезультатных поисков я обратился к властям";
					
			Link.l1.go = "portugalLineStart4";
			break;

		case "portugalLineStart4":
			d.Text = " Просто так оставить все дела и куда-то исчезнуть явно не входит в привычки сеньора Феррары, обычно он меня посвещает во все аспекты наших дел."
				+" И если бы он собирался куда-то отправиться, он бы непременно сообщил мне.";
			Link.l1 = "А куда он мог бы отправиться? Он мог покинуть остров? Насколько мне известно от губернатора, на Консейсао его следов не обнаружено.";
			Link.l1.go = "portugalLineStart5";
			break;

		case "portugalLineStart5":
			d.Text = "Если он и покинул наш остров, то скорее всего он бы отправился на Оксбэй. Кермит Праудфут - ростовщик в Гринфорде наш партнер, и я думаю вы можете поговорить с ним."
				+" Хотя если бы сеньор Феррара планировал встречу с сэром Праудфутом, то он бы мне это сказал.";
			Link.l1 = "Понятно, еще один вопрос, кем сеньору Ферраре приходится, сеньора Медейруш?";
			Link.l1.go = "portugalLineStart6";
			break;

		case "portugalLineStart6":
			d.Text = "Аманда Медейруш? Это его любовница. Это все что я о ней знаю. Когда исчез сеньор Феррара, она странно вела себя, а вскоре и вовсе покинула остров.";
			Link.l1 = "Вы сказали, что сеньора Медейруш странно вела себя, в чем это выражалось?";
			Link.l1.go = "portugalLineStart7";
			break;

		case "portugalLineStart7":
			d.Text = "Мне показалось, что она была чем-то взволнована.";
			Link.l1 = "Были ли какие-то странные события до исчезновения Феррары?";
			Link.l1.go = "portugalLineStart8";
			break;

		case "portugalLineStart8":
			d.Text = "Нет, думаю я сообщил все, что мне известно по этому поводу.";
			Link.l1 = "В таком случае, не буду больше отвлекать вас от дел.";
			Link.l1.go = "portugalLineStart9";
			break;

		case "portugalLineStart9":
			AddDialogExitQuest("GoToGreenfordBankir");
			AddQuestRecord("PortuguezeLine", “3”);
			DialogExit();
			break;
		}
}
