//nclude "DIALOGS\Quest_ANIMISTS_dialog.h"
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "First time":
			...
			if (pchar.quest.ANIMISTS == "letter_to_domingues")
			{
				...
				dialog.text = DLG_TEXT[2] + address_form.spa + DLG_TEXT[3];
				link.l1 = DLG_TEXT[4];
			...
			if (pchar.quest.ANIMISTS == "with_teacher_to_muelle" && pchar.location == "Denielle_deck")
			{
		...
		case "talk_in_shore_2":
			...
			dialog.text = DLG_TEXT[45] + address_form.spa + DLG_TEXT[46];
			link.l1 = DLG_TEXT[47];
			link.l1.go = "exit";			
