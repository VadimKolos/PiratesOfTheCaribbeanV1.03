//nclude "DIALOGS\Rabel Yverneau_dialog.h"
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "First time":
			...
			d.Text = DLG_TEXT[0];
			Link.l1 = DLG_TEXT[1] + NPChar.name + " " + Npchar.lastname + DLG_TEXT[2];
		...
		case "First time_1":
			...
			d.Text = DLG_TEXT[3];
			Link.l1 = Characters[GetCharacterIndex(DLG_TEXT[4])].name + DLG_TEXT[5];
		...
		case "First time_2":
			...
			d.Text = DLG_TEXT[6];
			Link.l1 = DLG_TEXT[7] + Pchar.name + " " + PChar.lastname + DLG_TEXT[8] + Characters[GetCharacterIndex(DLG_TEXT[9])].name + DLG_TEXT[10];
			Link.l1.go = "Node_2";	
		break;
		...
		case "Node_4":
			...
			d.Text = DLG_TEXT[15] + Characters[GetCharacterIndex(DLG_TEXT[16])].name + DLG_TEXT[17];
			Link.l1 = DLG_TEXT[18];
			Link.l1.go = "Node_5";
		break;
