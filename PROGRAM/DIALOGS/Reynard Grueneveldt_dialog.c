...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "quests":
			...
			if (!CheckAttribute(pchar, "quest.generate_kill_quest") && GetNationRelation2MainCharacter(HOLLAND) != RELATION_ENEMY)
			{
				dialog.text = DLG_TEXT[19];
				link.l3 = DLG_TEXT[20];
				link.l3.go = "kill_pirate";
			}			
		...
		case "kill_pirate":
			GeneratePirateQuest();
			dialog.text = DLG_TEXT[24] + characters[GetCharacterIndex(DLG_TEXT[25])].ship.name + DLG_TEXT[26] + LanguageConvertString(g_LocLngFileID, pchar.quest.killdestination) + DLG_TEXT[27] + pchar.quest.killmoney + DLG_TEXT[28];
			link.l1 = DLG_TEXT[29];
