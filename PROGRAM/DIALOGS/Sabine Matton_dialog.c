	...
	switch(Dialog.CurrentNode)
	{
		...
		case "First time":
			...
			if (npchar.quest.meeting == "0")
			{
				...
				Dialog.Text = DLG_TEXT[0] + npchar.name +  " " + npchar.lastname + DLG_TEXT[1] + npchar.nickname + DLG_TEXT[2];
				Link.l1 = pcharrepphrase(DLG_TEXT[3] + npchar.name + DLG_TEXT[4] + pchar.name + " " + pchar.lastname + DLG_TEXT[5], DLG_TEXT[6] + pchar.name + " " + pchar.lastname + DLG_TEXT[7] + pchar.name + DLG_TEXT[8]);
				Link.l1.go = "exit"; 
			...
			else
			{
				...
				Dialog.Text = DLG_TEXT[11] + pchar.name + DLG_TEXT[12];
		...
		case "Second Time":
			...
			if (npchar.quest.hire == "danielle_begin")
			{
				...
				dialog.text = DLG_TEXT[21] + pchar.name + DLG_TEXT[22];
				link.l1 = pcharrepphrase(DLG_TEXT[23], DLG_TEXT[24] + pchar.name + " " + pchar.lastname + DLG_TEXT[25]);
			...
			if (npchar.quest.hire == "request")
			{
				...
				link.l1 = pcharrepphrase(DLG_TEXT[27], DLG_TEXT[28] + pchar.name + " " + pchar.lastname + DLG_TEXT[29]);
				link.l1.go = "danielle_1";
			}
		break;
		...
		case "danielle_8":
			...
			dialog.text = DLG_TEXT[62] + pchar.name + DLG_TEXT[63];
		...
		case "danielle_end_2":
			...
			dialog.text = DLG_TEXT[68] + address_form.fra + DLG_TEXT[69];
			link.l1 = DLG_TEXT[70];
			link.l1.go = "exit";
		...
		case "smuggler_line":
			if (CheckQuestAttribute("smuggler_line", "fourth_mission_to_fdf"))
			{
				dialog.text = DLG_TEXT[74];
				link.l1 = DLG_TEXT[75];
				link.l1.go = "fourth_mission_to_fdf";
			}
		break;
		
		case "fourth_mission_to_fdf":
			dialog.text = DLG_TEXT[76];
			if (CheckCharacterItem(pchar, "Arnaud_ring"))
			{
				link.l1 = DLG_TEXT[77];
				link.l1.go = "fourth_mission_ring";
			}
			link.l2 = DLG_TEXT[78];
			link.l2.go = "fourth_mission_to_arnaud";
		break;
		...
		case "fourth_mission_to_arnaud":
			...
			link.l1 = DLG_TEXT[80];
			link.l1.go = "exit";
			AddDialogExitQuest("fourth_mission_to_arnaud");
		break;
		...
		case "fourth_mission_ring_right_6":
			...
			link.l1 = DLG_TEXT[97];
			link.l1.go = "exit";
			AddDialogExitQuest("fourth_mission_capture_sabine");
		break;
