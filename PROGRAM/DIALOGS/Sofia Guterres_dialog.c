//nclude "DIALOGS\Sofia Guterres_dialog.h"
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "First time":
			...
			if (pchar.quest.main_line == "night_for_rheims_house")
			{
				...
				dialog.text = DLG_TEXT[3] + address_form.por + DLG_TEXT[4];
		...
		case "node_2":
			...
			dialog.text = DLG_TEXT[11] + characters[GetcharacterIndex(DLG_TEXT[12])].name + " " + characters[GetcharacterIndex(DLG_TEXT[13])].lastname + DLG_TEXT[14] + address_form.por + DLG_TEXT[15];
			link.l1 = DLG_TEXT[16];
			link.l1.go = "sex";
			link.l2 = DLG_TEXT[17];
			link.l2.go = "node_3";
