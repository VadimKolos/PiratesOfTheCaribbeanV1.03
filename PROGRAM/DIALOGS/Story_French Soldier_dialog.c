//nclude "DIALOGS\Story_French Soldier_dialog.h"
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "first time":
			...
			Link.l2 = DLG_TEXT[2];
			Link.l2.go = "node_3";
			Link.l3 = Pchar.name + " " + PChar.lastname + DLG_TEXT[3];
		...
		case "node_2":
			...
			Link.l2 = DLG_TEXT[6];
			Link.l2.go = "node_4";
			Link.l3 = Pchar.name + " " + PChar.lastname + DLG_TEXT[7];
			Link.l3.go = "exit_fight";
		break;
		...
		case "node_4":
			if(makeint(PChar.skill.Leadership) >= 3)
			{
				Dialog.snd = "voice\STFR\STFR004";
				d.Text = DLG_TEXT[10] + address_form.fra + DLG_TEXT[11];
				Link.l1 = DLG_TEXT[12];
				Link.l1.go = "exit";
			}
			else
