...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "LoanGranted_exit":
			...
			Pchar.Quest.Loans.(NPC_Area).Sum = makeint(Pchar.Quest.Loans.(NPC_Area).Sum) + makeint(npchar.quest.loan);
			AddMoneyToCharacter(Pchar, makeint(npchar.quest.loan));
		...
		case "First time":
			...
			else
			{
					...
					if (makeint(npchar.quest.loan) < makeint(pchar.rank)*3000)
					{
		...
		case "small":
			npchar.quest.loan = 500*makeint(Pchar.rank);
			Dialog.snd = "voice\USDI\USDI017";
			d.Text = DLG_TEXT[69] + npchar.quest.loan + DLG_TEXT[70];
			Link.l1 = DLG_TEXT[71];
			Link.l1.go = "Interest";
			Link.l2 = DLG_TEXT[72];
			Link.l2.go = "Loan";
		...
		case "Medium":
			npchar.quest.loan = 1500*makeint(Pchar.rank);
			Dialog.snd = "voice\USDI\USDI018";
			d.Text = DLG_TEXT[74] + npchar.quest.loan + DLG_TEXT[75];
			Link.l1 = DLG_TEXT[76];
			Link.l1.go = "Interest";
			Link.l2 = DLG_TEXT[77];
			Link.l2.go = "Loan";
		...
		case "Large":
			npchar.quest.loan = 3000*makeint(Pchar.rank);
			Dialog.snd = "voice\USDI\USDI019";
			d.Text = DLG_TEXT[79] + npchar.quest.loan + DLG_TEXT[80];
			Link.l1 = DLG_TEXT[81];
			Link.l1.go = "Interest";
			Link.l2 = DLG_TEXT[82];
			Link.l2.go = "Loan";
