//nclude "DIALOGS\Vincent Bethune_dialog.h"
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "First time":
			...
			if (pchar.quest.main_line == "meet_Vincent_in_lighthouse")
			{
				...
				Dialog.Text = DLG_TEXT[2] + characters[GetCharacterIndex(DLG_TEXT[3])].name + DLG_TEXT[4];
				Link.l1 = DLG_TEXT[5] + npchar.name + DLG_TEXT[6] + pchar.name + " " + pchar.lastname + DLG_TEXT[7] + characters[GetCharacterIndex(DLG_TEXT[8])].name + DLG_TEXT[9];
		...
		case "meet_Vincent_in_lighthouse_2":
			...
			link.l1 = DLG_TEXT[27] + characters[GetCharacterIndex(DLG_TEXT[28])].name + DLG_TEXT[29];
