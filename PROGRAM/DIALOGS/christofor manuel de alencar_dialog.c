...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "kill_pirate":
			GeneratePirateQuest();
			dialog.text = DLG_TEXT[20] + characters[GetCharacterIndex(DLG_TEXT[21])].ship.name + DLG_TEXT[22] + LanguageConvertString(g_LocLngFileID, pchar.quest.killdestination) + DLG_TEXT[23] + pchar.quest.killmoney + DLG_TEXT[24];
			link.l1 = DLG_TEXT[25];
