//nclude "DIALOGS\Claudio Burrieza_dialog.h"
...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "second time":
			...
			dialog.text = randphrase(DLG_TEXT[6], DLG_TEXT[7] + address_form.fra + DLG_TEXT[8], DLG_TEXT[9], &dialog, dialog.snd1, dialog.snd2, dialog.snd3);
		...
		case "node_1":
			if (npchar.quest.meeting == "0")
			{
				...
				dialog.text = DLG_TEXT[14] + address_form.spa + DLG_TEXT[15] + npchar.name + " " + npchar.lastname + DLG_TEXT[16];
		...
		case "quests":
			...
			{
				link.l2 = DLG_TEXT[29] + characters[GetCharacterIndex(DLG_TEXT[30])].name + " " + characters[GetCharacterIndex(DLG_TEXT[31])].lastname + DLG_TEXT[32];
		...
		case "Danielle_find_FerroCerezo_1":
			if( CheckQuestAttribute("Story_DominguesCompromat","Find") )
			{
				...
				dialog.text = DLG_TEXT[70] + address_form.spa + DLG_TEXT[71];
				link.l1 = DLG_TEXT[72];
				link.l1.go = "exit";
			}
			else
			{
				...
				dialog.text = DLG_TEXT[73] + address_form.spa + DLG_TEXT[74];
				link.l1 = DLG_TEXT[75];
				link.l1.go = "exit";
		...
		case "pismena":
			...
			link.l1 = DLG_TEXT[80] + characters[getCharacterIndex(DLG_TEXT[81])].name + " " + characters[GetCharacterIndex(DLG_TEXT[82])].lastname + DLG_TEXT[83];
				...
				else
				{
					...
					else
					{
						...
						if (randomGod == 1)
						{
							...
							link.l1.go = "exit";
							pchar.quest.destination = "Muelle_port";
