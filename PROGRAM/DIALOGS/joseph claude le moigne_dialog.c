...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "quest":
			...
			if (!CheckAttribute(pchar, "quest.generate_kill_quest") && GetNationRelation2MainCharacter(FRANCE) != RELATION_ENEMY && npchar.quest.english_war_ship != "2" && npchar.quest.english_war_ship != "1")
			{
		...
		case "kill_pirate":
			GeneratePirateQuest();
			dialog.text = DLG_TEXT[41] + characters[GetCharacterIndex(DLG_TEXT[42])].ship.name + DLG_TEXT[43] + LanguageConvertString(g_LocLngFileID, pchar.quest.killdestination) + DLG_TEXT[44] + pchar.quest.killmoney + DLG_TEXT[45];
			link.l1 = DLG_TEXT[46];
