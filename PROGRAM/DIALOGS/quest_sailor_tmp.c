//nclude "DIALOGS\quest_sailor_tmp.h"
...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "First time":
			...
			{
				dialog.text = PChar.name+" "+PChar.lastname + DLG_TEXT[0];
				link.l1 = DLG_TEXT[1];
		...
		case "MissingBoatswain_1":
			dialog.text = DLG_TEXT[7] + Characters[GetCharacterIndex(DLG_TEXT[8])].name + DLG_TEXT[9];
			link.l1 = DLG_TEXT[10];
