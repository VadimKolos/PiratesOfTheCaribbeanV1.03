//nclude "DIALOGS\quest_smuggler_01_dialog.h"
...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "node_2":
			...
			dialog.text = DLG_TEXT[4] + characters[GetCharacterIndex(DLG_TEXT[5])].name + DLG_TEXT[6];
			link.l1 = DLG_TEXT[7];
			link.l1.go = "node_3";
		break;
		...
		case "node_5":
			...
			dialog.text = pcharrepphrase(DLG_TEXT[19] + characters[GetCharacterIndex(DLG_TEXT[20])].name + DLG_TEXT[21], DLG_TEXT[22]);
		...
		case "node_6":
			...
			dialog.text = pcharrepphrase(DLG_TEXT[29] +characters[GetCharacterIndex(DLG_TEXT[30])].name + DLG_TEXT[31], DLG_TEXT[32] + characters[GetCharacterIndex(DLG_TEXT[33])].lastname + DLG_TEXT[34]);
			link.l1 = DLG_TEXT[35];
			link.l1.go = "exit";
			AddDialogExitQuest("blaze_exit_to_machado");
