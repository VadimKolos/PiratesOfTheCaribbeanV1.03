//nclude "DIALOGS\researcher_dialog.h"
...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "First time":
			...
			if (pchar.quest.main_line == "without_ship")
			{
				...
				dialog.text = DLG_TEXT[15] + pchar.name + DLG_TEXT[16];
				link.l1 = DLG_TEXT[17];
		...
		case "without_ship":
			...
			link.l1 = DLG_TEXT[26] + characters[getCharacterindex(DLG_TEXT[27])].name + DLG_TEXT[28];
		...
		case "to_oxbay_by_prisoner_escape_researcher_2":
			...
			Link.l1 = DLG_TEXT[38] + pchar.name + " " + pchar.lastname + DLG_TEXT[39];
	...
	/*		
		...
		case "NeedSomeTime_Exit":
			...
			NPChar.quest.meeting = NPC_Meeting;
			DialogExit();
			DoQuestCheckDelay("Story_ReloadToTavernAndSpeakToDanielle", 1.0);
		break;
		...
		case "TempleWillBlast_1":
			...
			Diag.Tempnode = "LeaveTheFuckingKhaelRoa";
		...
		case "I go to the right_1":
			...
			d.Text = DLG_TEXT[50] + PChar.name + DLG_TEXT[51];
			Link.l1 = DLG_TEXT[52];
			Link.l1.go = "exit";
		...
		case "Translation":
			...
			d.Text = DLG_TEXT[67] + Pchar.name + DLG_TEXT[68];
			Link.l1 = DLG_TEXT[69];
		...
		case "Translation_1":
			...
			d.Text = DLG_TEXT[70];
			Link.l1 = DLG_TEXT[71] + NPchar.name + DLG_TEXT[72];
		...
		case "NeedSomeTime_2":
			...
			d.Text = DLG_TEXT[81] + PChar.name + DLG_TEXT[82];
			Link.l1 = DLG_TEXT[83];
		...
		case "NeedSomeTime_4":
			...
			d.Text = DLG_TEXT[86] + Pchar.name + DLG_TEXT[87];
			Link.l1 = DLG_TEXT[88];
