//nclude "DIALOGS\vigila mendes_dialog.h"
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "First time":
			...
			if (npchar.quest.meeting == "0")
			{
				...
				{
					link.l1 = pcharrepphrase(DLG_TEXT[3] + npchar.name + " " + npchar.lastname + DLG_TEXT[4], DLG_TEXT[5] + pchar.name + DLG_TEXT[6]);
					link.l1.go = "node_3";
				}
			...
			else
			{
				...
				if ((npchar.quest.escort == "end")&&(npchar.location == "Muelle_store"))
				{
					...
					dialog.text = DLG_TEXT[17] + address_form.spa + DLG_TEXT[18];
		...
		case "node_1":
			...
			dialog.text = DLG_TEXT[21] + npchar.name + " " + npchar.lastname + DLG_TEXT[22];
			link.l1 = pcharrepphrase(DLG_TEXT[23] + pchar.name + " " + pchar.lastname + DLG_TEXT[24], DLG_TEXT[25] + pchar.name + DLG_TEXT[26] + pchar.name + " " + pchar.lastname + DLG_TEXT[27]);
			link.l1.go = "node_2";
		break;
		...
		case "node_2":
			...
			link.l1.go = "talk_1";
			link.l2 = pcharrepphrase(DLG_TEXT[31] + npchar.name + DLG_TEXT[32], DLG_TEXT[33]);
			link.l2.go = "exit";
		break;
		...
		case "node_3":
			...
			link.l1 = pcharrepphrase(DLG_TEXT[35] + pchar.name + " " + pchar.lastname + DLG_TEXT[36], DLG_TEXT[37]);
			link.l1.go = "talk_3";
		break;
		...
		case "talk_1":
			if (makeint(pchar.reputation) < 40)
			{
				...
				dialog.text = DLG_TEXT[38] + address_form.spa + DLG_TEXT[39];
				link.l1 = pcharrepphrase(DLG_TEXT[40], DLG_TEXT[41]);
				link.l1.go = "node_exit";
			}
			if ((npchar.quest.escort == "0")&&(makeint(pchar.reputation) >= 40))
			{
				Dialog.snd = "voice\VIME\VIME010";
		...
		case "talk_3":
			...
			link.l1.go = "talk_4";
			if (GetSummonSkillFromName(pchar, "commerce") > 4)
			{
		...
		case "money":
			...
			dialog.text = DLG_TEXT[64] + (2500 + GetSummonSkillFromName(pchar, DLG_TEXT[65])*100) + DLG_TEXT[66];
			link.l1 = pcharrepphrase(DLG_TEXT[67], DLG_TEXT[68]);
			link.l1.go = "talk_4";
			link.l2 = pcharrepphrase(DLG_TEXT[69], DLG_TEXT[70]);
			link.l2.go = "talk_exit";
			npchar.money.quest = 2500 + GetSummonSkillFromName(pchar, "commerce")*100;
		break;
