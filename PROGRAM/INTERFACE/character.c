...
void InitInterface_R(string iniName,ref pCharacter)
{
    ...
    CreateString(true,"FreeAbilitiesPoints","",FONT_BOLD_NUMBERS,COLOR_NORMAL,610,394,SCRIPT_ALIGN_RIGHT,1.0);
    CreateString(true,"FreeSkillPoints","",FONT_BOLD_NUMBERS,COLOR_NORMAL,610,428,SCRIPT_ALIGN_RIGHT,1.0);
...
void SetNextParty(bool bRight)
{
	...
	if(bRight)
	{
		...
		{
			if(i<4) {
				cn = GetOfficersIndex(xi_refMainChar,i);
				if( cn>-1 ) break;
			} else {
				cn = GetCompanionIndex(xi_refMainChar,i-3);
				if( cn>-1) break;
			}
		}
		if(i<7)
