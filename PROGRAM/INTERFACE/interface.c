...
void ILaunchAfterFrame()
{
	...
	break;
	case "TransferMain":
		if( CheckAttribute(GetMainCharacter(),"TransferChar") )
		{
			LaunchTransferMain(GetMainCharacter(), GetCharacter(sti(Characters[GetMainCharacterIndex()].TransferChar)), "");
			DeleteAttribute( GetMainCharacter(), "TransferChar" );
		}
		return;
	break;
	}
