...
void ProcessCommandExecute()
{
	...
	case "ARCADEMODE_1":
		if(comName=="activate" || comName=="click")
		{
			if (bArcadeSailing == true)
			{
				bArcadeSailing = false;
			}
			else
			{
				bArcadeSailing = true;
			}
			TmpI_ShowArcadeMode1();
		}
	break;
	...
	case "ARCADEMODE_2":
		if(comName=="activate" || comName=="click")
		{
			if (bArcadeFencing == true)
			{
				bArcadeFencing = false;
			}
			else
			{
				bArcadeFencing = true;
			}
			TmpI_ShowArcadeMode2();
		}
	break;
	...
	case "ARCADEMODE_3":
		if(comName=="activate" || comName=="click")
		{
			if (bArcadeCannons == true)
			{
				bArcadeCannons = false;
			}
			else
			{
				bArcadeCannons = true;
			}
			TmpI_ShowArcadeMode3();
		}
	break;
	...
	break;
	case "CONTROLS_B_ALWAYSRUN":
		if(comName=="activate" || comName=="click")
		{
			if( CheckAttribute(&InterfaceStates,"alwaysrun") && sti(InterfaceStates.alwaysrun)==true )
			{MakeAlwaysRun(false);} else {MakeAlwaysRun(true);}
		}
		if(comName=="downstep" && GetTargetPlatform()=="pc")
		{
			SetCurrentNode("CONTROLS_B_INVERT");
...
void TmpI_ShowArcadeMode1()
{
	trace(bArcadeSailing)
	if(bArcadeSailing) 
	{
		CreateImage("ArcadeModeImg1","icons","complete",532,152,564,184);
	} 
	else 
	{
		CreateImage("ArcadeModeImg1","","",532,152,564,184);
	}
}

void TmpI_ShowArcadeMode2()
{
	if(bArcadeFencing) {
		CreateImage("ArcadeModeImg2","icons","complete",532,198,564,230);
	} else {
		CreateImage("ArcadeModeImg2","","",0,0,0,0);
	}
}

void TmpI_ShowArcadeMode3()
{
	if(bArcadeCannons) {
		CreateImage("ArcadeModeImg3","icons","complete",532,244,564,276);
	} else {
		CreateImage("ArcadeModeImg3","","",0,0,0,0);
	}
}
...
void ShowOptionsPage(int pageNum)
{
	...
	case OPAGE_SAILING:
		...
		SetNodeUsing("ARCADEMODE_3",false);
		GameInterface.pictures.ArcadeModeImg1.pic = -1;
		GameInterface.pictures.ArcadeModeImg1.tex = -1;
		GameInterface.pictures.ArcadeModeImg2.pic = -1;
		GameInterface.pictures.ArcadeModeImg2.tex = -1;
		GameInterface.pictures.ArcadeModeImg3.pic = -1;
		GameInterface.pictures.ArcadeModeImg3.tex = -1;
	...
	case OPAGE_CONTROLS_1:
		...
		SetNodeUsing("CONTROLS_B_ALWAYSRUN",false);
		...
		GameInterface.pictures.AlwaysRunImg.pic = -1;
		GameInterface.pictures.AlwaysRunImg.tex = -1;
		SetCurrentNode("MB_CONTROLS");
	break;
	...
	case OPAGE_SAILING:
		...
		SetNodeUsing("ARCADEMODE_3",true);
		TmpI_ShowArcadeMode1();
		TmpI_ShowArcadeMode2();
		TmpI_ShowArcadeMode3();
	...
	case OPAGE_CONTROLS_1:
		...
		SetNodeUsing("CONTROLS_B_ALWAYSRUN",true);
...
{
	if( CheckAttribute(&InterfaceStates,"alwaysrun") && sti(InterfaceStates.alwaysrun)==true ) {
		CreateImage("AlwaysRunImg","icons","complete",532,230,564,262);
	} else {
		CreateImage("AlwaysRunImg","","",0,0,0,0);
	}
}
...
void SetAlwaysRun(bool bRun)
{
	...
	if( CheckAttribute(&InterfaceStates,"alwaysrun") && sti(InterfaceStates.alwaysrun) )
	{
