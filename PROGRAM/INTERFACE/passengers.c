...
{
	int RansomCost = makeint(refCharacter.rank)*makeint(characters[0].skill.commerce)*100;
	return RansomCost;
}
...
{
	GameInterface.strings.CharacterName = chref.name + " " + chref.lastname;
	GameInterface.strings.CharacterNation = GlobalStringConvert(Nations[sti(chref.nation)].Name);
