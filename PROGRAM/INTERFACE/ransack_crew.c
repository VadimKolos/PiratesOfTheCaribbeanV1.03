...
void InitInterface_RR(string iniName,ref myCh,ref enemyCh)
{
	...
	CreateString(TRUE,"String3", nSurrenderedMen+" " + XI_ConvertString(str3_1)+ " " +XI_ConvertString(str3_2),FONT_NORMAL,COLOR_NORMAL,320,296,SCRIPT_ALIGN_CENTER,1.0);
...
void SlavesProcess()
{
	int iCargoType = GetCharacterFreeSpace(refMyCharacter, GOOD_SLAVES);
	
	if (nSurrenderedMen > iCargoType)
	{
		nSurrenderedMen = iCargoType)
	}
		
	AddCharacterGoods(refMyCharacter, GOOD_SLAVES, nSurrenderedMen);
	ProcessCancelExit();
}
