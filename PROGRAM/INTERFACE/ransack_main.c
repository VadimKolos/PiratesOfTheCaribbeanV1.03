...
bool g_bCHARACTERS_BUTTON = true;
bool g_bSWAP_BUTTON = true;
...
void WasChangeData()
{
	...
	if( g_bCHARACTERS_BUTTON ) {
		SetSelectable("CHARACTERS_BUTTON",GetCrewQuantity(refEnemyCharacter) >= GetMinCrewQuantity(refEnemyCharacter));
	}
	if( g_bSWAP_BUTTON ) {
		SetSelectable("SWAP_BUTTON",GetCrewQuantity(refMyCharacter) >= GetMinCrewQuantity(refEnemyCharacter));
	}
}
