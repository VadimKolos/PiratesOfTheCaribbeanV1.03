...
void InitInterface_B(string iniName, bool isSave)
{
	...
	CreateString(true,"SaveName","",FONT_NORMAL,COLOR_NORMAL,50,260,SCRIPT_ALIGN_LEFT,1.0);
	CreateString(true,"SaveTime","",FONT_NORMAL,COLOR_NORMAL,50,292,SCRIPT_ALIGN_LEFT,1.0);
...
void SettingDataForSave(string saveFileName)
{
	...
	string locationStr, timeStr, tmpStr, dateStr;
	int locidx;
	...
	if( saveFileName=="NewSave" )
	{
		...
		SendMessage(&GameInterface, "lsee", MSG_INTERFACE_GETTIME, "", &tmpStr, &dateStr);
	}
	else
	{
		...
		if( GetTargetPlatform()=="xbox" ) {
			SendMessage(&GameInterface, "lsee", MSG_INTERFACE_GETTIME, saveFileName, &tmpStr, &dateStr);
		} else {
			SendMessage(&GameInterface, "lsee", MSG_INTERFACE_GETTIME, GameInterface.SavePath+"\"+saveFileName, &tmpStr, &dateStr);
		}
	}
	...
	GameInterface.strings.SaveTime = dateStr + "   " + tmpStr;
...
string GetCurLocationName()
{
	...
	if( locidx>=0 )
	{
		if( CheckAttribute(&Locations[locidx],"id.label") ) {
			locLabel = LanguageConvertString(g_nLablesFileID,Locations[locidx].id.label);
		}
	}
	else
	{
