    ...
    if(sti(GameInterface.FourImage.current)!=nCurFourNum)
    {
		...
		if(shipType!=SHIP_NOTUSED)
		{
			...
			if(cn==-1)
			{
				...
				//CreateString(true,"ShipName",refBaseShip.Name,FONT_NORMAL,COLOR_NORMAL,GetXForShipName(),GetYForShipName(),SCRIPT_ALIGN_CENTER,1.0);
				CreateString(true,"TNHull",makeint(otherCh.ship.hp) + "/"+refBaseShip.hp,FONT_NORMAL,COLOR_NORMAL,63,58,SCRIPT_ALIGN_CENTER,1.0);
			}
			else
			{
...
void SetNoneData()
{
	...
	if(shipType!=SHIP_NOTUSED)
	{
		...
		GameInterface.strings.ShipManeuver = fts(stf(refBaseShip.TurnRate),3);
...
void SetCannonsData()
{
	...
	{
		GameInterface.strings.CannonsName = XI_ConvertString("noneCanon");
