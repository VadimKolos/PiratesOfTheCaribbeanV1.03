...
void InitInterface(string iniName)
{
	...
	SetEventHandler("evStartExchange","procStartExchange",0);
}

void RightToExchange()
{
	if(GetSelectable("EXCHANGE_BUTTON")==true) { SetCurrentNode("EXCHANGE_BUTTON"); }
...
void ProcessFrame()
{
    ...
    if(sti(GameInterface.FourImage.current)!=nCurFourNum)
    {
		...
		refCharacter = GetMainCharacter();
		SetSelectable("EXCHANGE_BUTTON",false);
		if(nCurFourNum>0)
		{
			...
			else
			{
				...
				if( GetRemovable(refCharacter) ) {
					SetSelectable("EXCHANGE_BUTTON",true);
				}
			}
		}
...
void IDoExit()
{
	...
	DelEventHandler("evStartExchange","procStartExchange");
...
void SetShowType(int newShowType)
{
	...
	case SHOW_TYPE_MAIN:
		...
		SetNodeUsing("EXCHANGE_BUTTON",false);
	...
	case SHOW_TYPE_MAIN:
		...
		SetNodeUsing("EXCHANGE_BUTTON",true);
...
void ProcessCommandExecute()
{
	...
	case "DISCARD_BUTTON":
		...
		if(comName=="rightstep")	{RightToExchange();}
	break;

	case "INFO_BUTTON":
		if(comName=="activate" || comName=="click")	{ProcessInfo();}
		if(comName=="leftstep")	{LeftToExchange();}
	break;
...
void procStartExchange()
{
	nCurFourNum = (GameInterface.FourImage.current);
	if(nCurFourNum<1) return;

	int cn = GetCompanionIndex(GetMainCharacter(),nCurFourNum);
	if(cn<0) return;

	ref chref = GetCharacter(cn);
	if( !GetRemovable(chref) ) return;

	Characters[GetMainCharacterIndex()].TransferChar = cn;
	InterfaceStates.InstantExit = true;
	ProcessCancelExit();
	PostEvent("LaunchIAfterFrame",1,"sl","TransferMain",1);
}
