...
void InitInterface_R(string iniName,ref shipmaster)
{
    ...
    CreateString(true,"ship0_1","",FONT_NORMAL,COLOR_NORMAL,80,296,SCRIPT_ALIGN_CENTER,1.0);
    CreateString(true,"ship0_2","",FONT_NORMAL,COLOR_NORMAL,80,274,SCRIPT_ALIGN_CENTER,1.0);
    CreateString(true,"ship1_1","",FONT_NORMAL,COLOR_NORMAL,560,296,SCRIPT_ALIGN_CENTER,1.0);
    CreateString(true,"ship1_2","",FONT_NORMAL,COLOR_NORMAL,560,274,SCRIPT_ALIGN_CENTER,1.0);
    CreateString(true,"ship2_1","",FONT_NORMAL,COLOR_NORMAL,80,444,SCRIPT_ALIGN_CENTER,1.0);
    CreateString(true,"ship2_2","",FONT_NORMAL,COLOR_NORMAL,80,422,SCRIPT_ALIGN_CENTER,1.0);
    CreateString(true,"ship3_1","",FONT_NORMAL,COLOR_NORMAL,560,444,SCRIPT_ALIGN_CENTER,1.0);
    CreateString(true,"ship3_2","",FONT_NORMAL,COLOR_NORMAL,560,422,SCRIPT_ALIGN_CENTER,1.0);
...
int GetSellPrice(int fourIdx)
{
	...
	{
		price = (price*(100-GetHullDamagePercent(fourIdx)))/100;
		price = (price*(200-GetSailDamagePercent(fourIdx)))/200;
	}
	price /= 3;
...
void SetCaptainButtonsEnable()
{
	...
	int cn = GetCompanionIndex(GetMainCharacter(),nCurFourNum);
	if( cn<0 || !GetRemovable(GetCharacter(cn)) || GetCharacterShipType(GetCharacter(cn))!=SHIP_NOTUSED )	bRemoveEnable = false;
...
void SplitStringForWidth(string strSrc, int width, string fontName, float fontScale, ref strDest, ref strRemnant)
{
	int nW = GetStringWidth(strSrc, fontName, fontScale);

	if(nW>width)
	{
		int nStrSize = strlen(strSrc);
		int fndNum = findSubStr(strSrc," ",0);
		string tmpStr;
		while(fndNum>=0)
		{
			tmpStr = strcut(strSrc,fndNum+1,nStrSize-1);
			if( GetStringWidth(tmpStr, fontName, fontScale) <= width )
			{
				strRemnant = strcut(strSrc,0,fndNum-1);
				strDest = tmpStr;
				return;
			}
			fndNum = findSubStr(strSrc," ",fndNum+1);
		}
	}
...
void UpdateFourShipType()
{
	int i,cn;
	string strName1,strName2;
	string shipTypeName1,shipTypeName2;
	ref mch, cref;

	int x,y1,y2;

	mch = GetMainCharacter();

	for(i=0; i<4; i++)
	{
		strName1 = "ship"+i+"_1";
		strName2 = "ship"+i+"_2";
		cn = GetCompanionIndex(mch,i);
		if(cn>=0)
		{
			SplitStringForWidth(XI_ConvertString(GetShipTypeName(GetCharacter(cn))),128,FONT_NORMAL,1.0,&shipTypeName1,&shipTypeName2);
			GameInterface.strings.(strName1) = shipTypeName1;
			GameInterface.strings.(strName2) = shipTypeName2;
		} else {
			GameInterface.strings.(strName1) = "";
			GameInterface.strings.(strName2) = "";
		}
	}
}
