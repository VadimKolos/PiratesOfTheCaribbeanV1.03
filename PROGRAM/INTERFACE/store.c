...
void ProcessOk()
{
	...
	if(showType==SHOW_BUY)
	{
		...
		nPlayerMoney2 = GetGoodPriceByType(GetGoodsIndexForI(nCurScrollNum),nTradeQuantity,GetStoreGoodsPrice(refStore,GetGoodsIndexForI(nCurScrollNum),PRICE_TYPE_BUY,refCharacter));
		nPlayerMoney2 = -nPlayerMoney2;
	...
	if(showType==SHOW_SELL)
	{
		...
		nPlayerMoney2 = GetGoodPriceByType(GetGoodsIndexForI(nCurScrollNum),nTradeQuantity,GetStoreGoodsPrice(refStore,GetGoodsIndexForI(nCurScrollNum),PRICE_TYPE_SELL,refCharacter));
	}
	//ref tmpref = GetMainCharacter();
	//tmpref.Money = nPlayerMoney;
	AddMoneyToCharacter(&characters[0], nPlayerMoney2);
	nPlayerMoney = characters[0].money;
    ScrollChange();
