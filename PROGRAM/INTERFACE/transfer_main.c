...
void InitInterface_RRS(string iniName,ref myCh,ref enemyCh,string newCurNod)
{
	...
	{
		trace("WARNING!!! You hav`t companion id = " + enemyCh.id);
	...
	if(!GetSelectable(newCurNod))	DownProcess();
}
...
void ProcessCancelExit()
{
	...
	if(n<0)
	{
		if( bSeaActive )
		{
			...
			ShipDead(sti(tmpChRef.index),KILL_BY_ABORDAGE,sti(refMyCharacter.index));
		}
