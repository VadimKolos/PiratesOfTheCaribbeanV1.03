...
void LAi_StartBoarding(int locType, ref echr, bool isMCAttack)
{
	...
	float moral;
	if (CheckCharacterPerk(mchr, "MusketsShoot"))
	{
		int musketminus = ecrew*(25-makeint(echr.skill.defence))/100;
		ecrew = ecrew - musketminus;
	}
	...
	if (CheckForSurrender(mchr, echr))
	{
		...
		if(sti(mchr.ship.crew.morale)>99) { mchr.ship.crew.morale=99; }
	...
	while(maxcrew < BRDLT_MAXCREW)
	{
		...
		if(locID < 0) break;
		addcrew = MAX_GROUP_SIZE;
		maxcrew = maxcrew + addcrew;
	}
	//Отношение сторон
	...
	for(int i = 0; i < LAi_numloginedcharacters; i++)
	{
		...
		if(index >= 0)
		{
			if(index != GetMainCharacterIndex())
			{
				...
				if(LAi_IsDead(chr) == false == false && IsOfficer(chr) == false && IsCompanion(chr) == false )
				{
...
int GetAdjustedMorale(ref mchr, ref echr, bool FinalAdjust)
{
	...
	if(mclass>eclass) { morale = morale-((mclass-eclass)*2); } // Slightly nervous but not overmuch
	else if(mclass<eclass) { morale = morale+((mclass-eclass)*10); } // Ohhh, you are soooo fucking DEAD!

	if(((mCrewQ+eCrewQ)>sti(mstype.MaxCrew)) && mreput<31) { morale=morale+makeint(rand(10)+10); } // See note #2 at function end
	morale=morale-makeint(100-GetHullPercent(echr));
	...
	if(mfight<efight) { morale = morale + 5; }
	if(FinalAdjust) { morale=morale-losing; } // See note #5 at function end
...
void SetEndingStats()
{
	...
	if(PlayerChar.ship.crew.morale>99) { PlayerChar.ship.crew.morale = 99; }
