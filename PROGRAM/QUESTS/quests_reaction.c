void GeneratePirateQuest()
{
	ref PChar;
	PChar = GetMainCharacter();
	int iShipType, iTradeMoney, irank;
	string sdestination;
	LAi_SetHp(characterFromID("Quest pirate"), 80.0, 80.0);
	
	irank = PChar.rank;
	DeleteAttribute(characterFromID("Quest pirate"), "Ship");
	
	if (irank < 4)
	{
		iShipType = makeint(rand(4))+1;
		characters[GetCharacterIndex("Quest pirate")].ship.type = iShipType;
	}
	if (irank >= 4 && irank < 7)
	{
	...
	if (characters[GetCharacterIndex("Quest pirate")].ship.type >50)
	{
		characters[GetCharacterIndex("Quest pirate")].ship.type = 23;
	}
	SetRandomNameToShip(characterFromID("Quest pirate"));
	SetBaseShipData(characterFromID("Quest pirate"));
	...
	sdestination = GenerateDestinationKill();
	
	pchar.quest.killdestination = sdestination; 
}
...
void GiveItemToTrader(aref ch)
{
	...
	if (CheckAttribute(pchar, "quest.Tut_BuySpyGlass")) { TakeNItems(ch,"spyglass2", 1);}
...
string GenerateDestinationKill()
{
	ref PChar;
	PChar = GetMainCharacter();
	...
	switch (destination)
	{
		case 0: sdestination = "Redmond"; break;
		case 1: sdestination = "Oxbay"; break;
		case 2: sdestination = "IslaMuelle"; break;
		case 3: sdestination = "Conceicao"; break;
		case 4: sdestination = "Douwesen"; break;
		case 5: sdestination = "FalaiseDeFleur"; break;
	}
	return sdestination;
}
...
string GenerateDestination()
{
	...
	if (destination == 5) 
	{
		...
		{
			sdestination = "Falaise_De_Fleur_port_01";
		}
	}
	return sdestination;
}
...
void GenerateConvoyQuest()
{
	...
	pchar.quest.generate_convoy_quest.win_condition.l1.date.day = GetAddingDataDay(0, 0, 30);
    pchar.quest.generate_convoy_quest.win_condition.l1.date.month = GetAddingDataMonth(0, 0, 30);
	pchar.quest.generate_convoy_quest.win_condition.l1.date.year = GetAddingDataYear(0, 0, 30);
	...
	{
		sDestination = "Conceicao_shore_02";
	}
...
void QuestComplete(string sQuestName)
{
	...
	switch(sQuestName)
	{
		...
		case "Story_leavingOxbay":
			...
			//Заведение квеста на встречу с Амандой в порту Консейсао
			pchar.quest.AmandaQuest.win_condition.l1 = "location";
			pchar.quest.AmandaQuest.win_condition.l1.location = "Conceicao_port";
			pchar.quest.AmandaQuest.win_condition = "AmandaQuest";
		...
		case "Massoni_spared":
			...
			Pchar.quest.Story_Capture_Oiseau.win_condition.l1 = "Character_Capture";
			Pchar.quest.Story_Capture_Oiseau.win_condition.l1.character = "Virgile Boon";
			Pchar.quest.Story_Capture_Oiseau.win_condition = "Story_Capture_Oiseau";
		...
		case "Story_Sink_Oiseau":
			pchar.quest.Story_Capture_Oiseau.over = "yes";
		...
		case "Story_Capture_Oiseau":
			pchar.quest.Story_Sink_Oiseau.over = "yes";
			SetNationRelation2MainCharacter(ENGLAND, RELATION_FRIEND);
			SetCharacterShipLocation(characterFromID("Virgile boon"), "none");
			pchar.quest.Convoy_Virgile_Boon.over = "yes";
			pchar.quest.Convoy_Virgile_Boon2.over = "yes";
		...
		case "Story_Blaze_reaches_Greenford":
			...
			{
				AddQuestRecord("Story_2ndTask", "5");
		...
		case "Story_BlazeLeavesDanielleRoom":
			...
			ExchangeCharacterShip(Pchar, characterFromID("Ship Storage"));
			DeleteAttribute(characterFromID("Ship Storage"), "ship");
			characters[GetCharacterIndex("ship storage")].ship.type = SHIP_NOTUSED;
			SetBaseShipData(characterFromID("ship storage"));
			pchar.location.from_sea = "Muelle_port";
			pchar.money = makeint(pchar.money) + makeint(pchar.tempmoney);
		...
		case "Story_DanielleGoesToBlazeShip":
			...
			characters[GetCharacterIndex("Danielle")].rank = makeint(pchar.rank);
			iPassenger = (makeint(characters[GetCharacterIndex("Danielle")].rank)*4)+100;
			LAi_SetHp(characterFromID("Danielle"), iPassenger, iPassenger);
			iPassenger = makeint(characters[characterFromID("Danielle")].skill.Leadership) + makeint(characters[characterFromID("Danielle")].skill.Fencing) + makeint(characters[characterFromID("Danielle")].skill.Sailing) + makeint(characters[characterFromID("Danielle")].skill.Accuracy) + makeint(characters[characterFromID("Danielle")].skill.Cannons) + makeint(characters[characterFromID("Danielle")].skill.Grappling) + makeint(characters[characterFromID("Danielle")].skill.Repair) + makeint(characters[characterFromID("Danielle")].skill.Defence) + makeint(characters[characterFromID("Danielle")].skill.Commerce) + makeint(characters[characterFromID("Danielle")].skill.Sneak);
			if (iPassenger < (makeint(characters[characterFromID("Danielle")].rank)*2))
			{
				iPassenger = makeint(characters[characterFromID("Danielle")].rank) - iPassenger;
				characters[characterFromID("Danielle")].skill.freeskill = iPassenger;
			}	
			characters[GetCharacterIndex("danielle")].perks.list.BasicDefense = true;
			characters[GetCharacterIndex("danielle")].perks.list.AdvancedDefense = true;
			if (makeint(characters[characterFromID("Danielle")].rank) > 10)
			{
				iPassenger = makeint(characters[characterFromID("Danielle")].rank) - 10;
				characters[characterFromID("Danielle")].perks.freepoints = iPassenger;
			}
		...
		case "Story_MovingToLighthouse":
			...
			pchar.quest.capture_greenford = "captured";
		break;
		...
		case "Story_RepelEnglishAssaultOnGreenford":
			if(!CheckAttribute(pchar, "sink_RepelEnglishAssaultOnGreenford"))
			{
				pchar.sink_RepelEnglishAssaultOnGreenford = 1;
			}
			else
			{
				pchar.sink_RepelEnglishAssaultOnGreenford = makeint(pchar.sink_RepelEnglishAssaultOnGreenford) + 1;
			}
			if (makeint(pchar.sink_RepelEnglishAssaultOnGreenford) >=3)
			{
				DisableFastTravel(true);
				DisableMenuLaunch(true);
				Island_SetReloadEnableGlobal("oxbay",true);
				//DoQuestCheckDelay("Story_RepelEnglishAssaultOnGreenford_2", 5.0);
				DeleteAttribute(pchar, "sink_RepelEnglishAssaultOnGreenford");
				pchar.quest.Story_RepelEnglishAssaultOnGreenford_2.win_condition.l1 = "ExitFromLocation";
				pchar.quest.Story_RepelEnglishAssaultOnGreenford_2.win_condition.l1.location = "Oxbay";
				pchar.quest.Story_RepelEnglishAssaultOnGreenford_2.win_condition = "Story_RepelEnglishAssaultOnGreenford_2";
		...
		case "Story_RepelEnglishAssaultOnGreenford_2":
			PlaceCharacter(characterFromID("danielle"), "goto", pchar.location);
		...
		case "Story_VoyageToKhaelRoaBegan":
			...
			Group_LockTask("Story_Pirate_Squadron");

			Pchar.quest.Story_SinkJurcksenAtKhaelRoa.win_condition.l1 = "NPC_Death";
			Pchar.quest.Story_SinkJurcksenAtKhaelRoa.win_condition.l1.character = "Isenbrandt Jurcksen";
			Pchar.quest.Story_SinkJurcksenAtKhaelRoa.win_condition = "Story_SinkPiratesAtKhaelRoa";
			
			Pchar.quest.Story_SinkSlaverAtKhaelRoa.win_condition.l1 = "NPC_Death";
			Pchar.quest.Story_SinkSlaverAtKhaelRoa.win_condition.l1.character = "Brian The Slaver";
			Pchar.quest.Story_SinkSlaverAtKhaelRoa.win_condition = "Story_SinkPiratesAtKhaelRoa";
			
			Pchar.quest.Story_SinkStrijdomAtKhaelRoa.win_condition.l1 = "NPC_Death";
			Pchar.quest.Story_SinkStrijdomAtKhaelRoa.win_condition.l1.character = "Femensy Strijdom";
			Pchar.quest.Story_SinkStrijdomAtKhaelRoa.win_condition = "Story_SinkPiratesAtKhaelRoa";
			
			Pchar.quest.Story_SinkSilehardAtKhaelRoa.win_condition.l1 = "NPC_Death";
			Pchar.quest.Story_SinkSilehardAtKhaelRoa.win_condition.l1.character = "Robert Christopher Silehard";
			Pchar.quest.Story_SinkSilehardAtKhaelRoa.win_condition = "Story_SinkPiratesAtKhaelRoa";
			
			//
			
			Pchar.quest.Story_CaptureJurcksenAtKhaelRoa.win_condition.l1 = "Character_Capture";
			Pchar.quest.Story_CaptureJurcksenAtKhaelRoa.win_condition.l1.character = "Isenbrandt Jurcksen";
			Pchar.quest.Story_CaptureJurcksenAtKhaelRoa.win_condition = "Story_SinkPiratesAtKhaelRoa";
			
			Pchar.quest.Story_CaptureSlaverAtKhaelRoa.win_condition.l1 = "Character_Capture";
			Pchar.quest.Story_CaptureSlaverAtKhaelRoa.win_condition.l1.character = "Brian The Slaver";
			Pchar.quest.Story_CaptureSlaverAtKhaelRoa.win_condition = "Story_SinkPiratesAtKhaelRoa";
			
			Pchar.quest.Story_CaptureStrijdomAtKhaelRoa.win_condition.l1 = "Character_Capture";
			Pchar.quest.Story_CaptureStrijdomAtKhaelRoa.win_condition.l1.character = "Femensy Strijdom";
			Pchar.quest.Story_CaptureStrijdomAtKhaelRoa.win_condition = "Story_SinkPiratesAtKhaelRoa";
			
			Pchar.quest.Story_CaptureSilehardAtKhaelRoa.win_condition.l1 = "Character_Capture";
			Pchar.quest.Story_CaptureSilehardAtKhaelRoa.win_condition.l1.character = "Robert Christopher Silehard";
			Pchar.quest.Story_CaptureSilehardAtKhaelRoa.win_condition = "Story_SinkPiratesAtKhaelRoa";
		...
		case "Story_SinkPiratesAtKhaelRoa":
			if(!CheckAttribute(pchar, "sink_pirates_at_khael_roa"))
			{
				pchar.sink_pirates_at_khael_roa = 1;
			}
			else
			{
				pchar.sink_pirates_at_khael_roa = makeint(pchar.sink_pirates_at_khael_roa) + 1;
			}
			if (makeint(pchar.sink_pirates_at_khael_roa) >=4)
			{
				Island_SetReloadEnableLocal("KhaelRoa", "reload_1", true);
				...
				DeleteAttribute(Pchar, "sink_pirates_at_khael_roa"); 
			}
		break;
		
		case "Story_LandedOnKhaelRoa":
			pchar.quest.fix_bag_with_save_in_labirint.win_condition.l1 = "location";
			pchar.quest.fix_bag_with_save_in_labirint.win_condition.l1.location = "labirint_1";
			pchar.quest.fix_bag_with_save_in_labirint.win_condition = "fix_bag_with_save_in_labirint";
		...
		case "fix_bag_with_save_in_labirint":
		...
		case "to_suadron":
			...
			Pchar.quest.Story_RepelTomlisonOnGreenford.win_condition.l1 = "NPC_Death";
			Pchar.quest.Story_RepelTomlisonOnGreenford.win_condition.l1.character = "Waulter Tomlison";
			Pchar.quest.Story_RepelTomlisonOnGreenford.win_condition = "Story_RepelEnglishAssaultOnGreenford";
			
			Pchar.quest.Story_RepelHartOnGreenford.win_condition.l1 = "NPC_Death";
			Pchar.quest.Story_RepelHartOnGreenford.win_condition.l1.character = "Malcolm Hart";
			Pchar.quest.Story_RepelHartOnGreenford.win_condition = "Story_RepelEnglishAssaultOnGreenford";
			
			Pchar.quest.Story_RepelBanfieldOnGreenford.win_condition.l1 = "NPC_Death";
			Pchar.quest.Story_RepelBanfieldOnGreenford.win_condition.l1.character = "Henry Banfield";
			Pchar.quest.Story_RepelBanfieldOnGreenford.win_condition = "Story_RepelEnglishAssaultOnGreenford";
			
			Pchar.quest.Story_CaptureTomlisonOnGreenford.win_condition.l1 = "Character_Capture";
			Pchar.quest.Story_CaptureTomlisonOnGreenford.win_condition.l1.character = "Waulter Tomlison";
			Pchar.quest.Story_CaptureTomlisonOnGreenford.win_condition = "Story_RepelEnglishAssaultOnGreenford";
			
			Pchar.quest.Story_CaptureHartOnGreenford.win_condition.l1 = "Character_Capture";
			Pchar.quest.Story_CaptureHartOnGreenford.win_condition.l1.character = "Malcolm Hart";
			Pchar.quest.Story_CaptureHartOnGreenford.win_condition = "Story_RepelEnglishAssaultOnGreenford";
			
			Pchar.quest.Story_CaptureBanfieldOnGreenford.win_condition.l1 = "Character_Capture";
			Pchar.quest.Story_CaptureBanfieldOnGreenford.win_condition.l1.character = "Henry Banfield";
			Pchar.quest.Story_CaptureBanfieldOnGreenford.win_condition = "Story_RepelEnglishAssaultOnGreenford";
		break;
		...
		case "post_use_artefact":
			...
			characters[GetCharacterindex("Barbossa")].ship.hp = makeint(pchar.ship.hp)*2 + iHP1 + iHP2 + iHP3;
		...
		case "end_game":
			pchar.quest.main_line = "completed";
			Characters[GetCharacterIndex("researcher")].Dialog.CurrentNode = "First time";
			Characters[GetCharacterIndex("danielle")].Dialog.CurrentNode = "First time";
			pchar.quest.danielle_death.over = "yes";
			SetCharacterRemovable(characterFromID("danielle"), true);
			SetCharacterRemovable(characterFromID("researcher"), true);
			characters[GetCharacterIndex("Isenbrandt Jurcksen")].model = "Depp";
			characters[GetCharacterIndex("Isenbrandt Jurcksen")].headModel = "h_depp";
			characters[GetCharacterIndex("Isenbrandt Jurcksen")].name = "Джек";
			characters[GetCharacterIndex("Isenbrandt Jurcksen")].lastname = "Воробей";
			LAi_SetHP(characterFromID("Isenbrandt Jurcksen"), 128.0, 128.0);
			characters[GetCharacterIndex("Isenbrandt Jurcksen")].dialog.currentnode = "third time";
			LaunchMainMenu();
		break;
		...
		case "exit_from_duarte_complete":
			LAi_RemoveLoginTime(characterFromID("quest_smuggler_01"));
		...
		case "blaze_first_speak_with_machado_complete":
			...
			LAi_ActorDialog(characterFromID("Camilo Machado"), pchar, "machado_back_to_hovernor_type", 20.0, 0);
			//начало линейки за контрабандистов
		...
		case "kill_ferro_cerezo_complete":
			pchar.quest.abording_ferro_cerezo_complete.over = "yes";
		...
		case "before_storm":	
			characters[getCharacterIndex("Greenford Commander")].skill.accuracy = "10";
			characters[getCharacterIndex("Greenford Commander")].skill.cannons = "10";

			LAi_group_SetRelation("DOUWESEN_PIRATE_SOLDIERS", LAI_GROUP_PLAYER, LAI_GROUP_NEITRAL);
			LAi_group_SetRelation("DOUWESEN_PIRATE_CITIZENS", LAI_GROUP_PLAYER, LAI_GROUP_NEITRAL);
			pchar.quest.go_to_pirate_fort3_complete.over = "yes";
			pchar.quest.go_to_pirate_fort1_complete.over = "yes";
			pchar.quest.go_to_pirate_fort2_complete.over = "yes";
		...
		case "turpin_cabanel_story_exit":
			...
			Locations[FindLocation("Falaise_de_fleur_port_01")].reload.l5.disable = 1;
		break;
		
		case "turpin_cabanel_story_exit_2":
			LAi_SetCitizenType(characterFromID("Tancrede Rimbaud"));
			LAi_type_actor_Reset(characterFromID("Virgile Boon"));
			LAi_ActorDialog(characterFromID("Virgile Boon"), Pchar, "open_gates_in_fdf_port", 25.0, 1.0);			
		break;
		
		case "open_gates_in_fdf_port":
			Locations[FindLocation("Falaise_de_fleur_port_01")].reload.l3.disable = 0;
			Locations[FindLocation("Falaise_de_fleur_port_01")].reload.l5.disable = 0;
		break;
		...
		case "thierry_bosquet_fight_completed":
			...
			ChangeCharacterAddress(characterFromID("thierry bosquet"), "none", ""));
			ChangeCharacterAddress(characterFromID("France_bukaner_01"), "none", ""));
		...
		case "boarding_smuggler_ship":
			pchar.quest.sink_smuggler_ship.over = "yes";
			bQuestDisableMapEnter = false;
		...
		case "sink_smuggler_ship":
			pchar.quest.boarding_smuggler_ship.over = "yes";
			bQuestDisableMapEnter = false;
			Island_SetReloadEnableGlobal("Redmond", true);
		break;
		...
		case "researcher_captured_exit_fight":
			characters[GetCharacterIndex("lighthouse_officer")].skill.fencing = "3";
			characters[GetCharacterIndex("Lighthouse_Guard_01")].skill.fencing = "2";
			characters[GetCharacterIndex("Lighthouse_Guard_02")].skill.fencing = "2";
			LAi_SetHp(characterFromID("lighthouse_officer"), 60, 60);
			LAi_SetHp(characterFromID("Lighthouse_Guard_01"), 60, 60);
			LAi_SetHp(characterFromID("Lighthouse_Guard_02"), 60, 60);
		...
		case "joseph_claude_le_mougne_english_war_ship":
			...
			pchar.quest.capture_pirate_near_fdf.win_condition.l1 = "Character_Capture";
			pchar.quest.capture_pirate_near_fdf.win_condition.l1.character = "Eng Captain Near FdF";
			pchar.quest.capture_pirate_near_fdf.win_condition = "capture_pirate_near_fdf";
		break;
		...
		case "Greenford_commander_fight_exit":
			...
			SetCharacterRelationBoth(GetCharacterIndex("Eng Captain Idol"),GetMainCharacterIndex(),RELATION_ENEMY);
			SetCharacterRelationBoth(GetCharacterIndex("Greenford Commander"),GetMainCharacterIndex(),RELATION_ENEMY);
		...
		case "to_greenford_for_montanez_complete":
			...
			LAi_setHp(characterFromID("Mystery_Man_01"), 80, 80);
			LAi_setHp(characterFromID("Mystery_Man_02"), 80, 80);
			...
			pchar.quest.animist1_drown.win_condition.l1 = "NPC_Death";
			pchar.quest.animist1_drown.win_condition.l1.character = "Mystery_Man_01";
			pchar.quest.animist1_drown.win_condition = "animists_drown_complete";
			
			pchar.quest.animist2_drown.win_condition.l2 = "NPC_Death";
			pchar.quest.animist2_drown.win_condition.l2.character = "Mystery_Man_02";
			pchar.quest.animist2_drown.win_condition = "animists_drown_complete";
			
			pchar.quest.animist1_capture.win_condition.l1 = "Character_Capture";
			pchar.quest.animist1_capture.win_condition.l1.character = "Mystery_Man_01";
			pchar.quest.animist1_capture.win_condition = "animists_drown_complete";
			
			pchar.quest.animist2_capture.win_condition.l2 = "Character_Capture";
			pchar.quest.animist2_capture.win_condition.l2.character = "Mystery_Man_02";
			pchar.quest.animist2_capture.win_condition = "animists_drown_complete";		
		break;
		...
		case "animists_drown_complete":
			if(!CheckAttribute(pchar, "sink_animists_at_greenford"))
			{
				pchar.sink_animists_at_greenford = 0;
			}
			else
			{
				pchar.sink_animists_at_greenford = makeint(pchar.sink_animists_at_greenford) + 1;
			}
			if (makeint(pchar.sink_animists_at_greenford) >=2)
			{
				DoQuestCheckDelay("animists_drown_complete_2", 5.0);
				AddQuestRecord("animists", "16");
				DeleteAttribute(pchar, "sink_animists_at_greenford");
			}
		break;
		...
		case "teacher_died":
			LAi_LoginInCaptureTown(characterFromID("Greenford Prison Commendant"), false);
		...
		case "fight_with_mefisto":
			...
			pchar.quest.capture_mefisto_complete.win_condition.l1 = "Character_Capture";
			pchar.quest.capture_mefisto_complete.win_condition.l1.character = "Dark Captain";
			pchar.quest.capture_mefisto_complete.win_condition = "destroy_mefisto_complete";
		...
		case "girl_exit_from_room_2_complete":
			...
			locations[FindLocation(pchar.location)].reload.l1.disable = 1;			
		break;
		...
		case "fight_with_larrouse_complete":
			...
			{
				iPassenger = makeint(characters[GetCharacterIndex("clair larrouse")].quest.goldmine)*10000;
		...
		case "exit_from_oxbay_to_sea_complete":
			...
			pchar.quest.capture_clair_in_sea.win_condition.l1 = "Character_Capture";
			pchar.quest.capture_clair_in_sea.win_condition.l1.character = "Clair Larrouse";
			pchar.quest.capture_clair_in_sea.win_condition = "killing_clair_in_sea_complete";
		...
		case "from_shore_to_ship":
			...
			LAi_SetOfficerType(characterFromID("clair larrouse"));
			characters[GetCharacterIndex("clair larrouse")].dialog.currentnode = "hired";
	...
	//Hitman
		...
		case "Hit_timer1":
			...
			PChar.quest.Hit_timer1.win_condition.l1.date.day = GetAddingDataDay(0, 0, 30);
			PChar.quest.Hit_timer1.win_condition.l1.date.month = GetAddingDataMonth(0, 0, 30);
			PChar.quest.Hit_timer1.win_condition.l1.date.year = GetAddingDataYear(0, 0, 30);
		...
		case "Hit_timer2":
			...
			PChar.quest.Hit_timer2.win_condition.l1.date.day = GetAddingDataDay(0, 0, 30);
			PChar.quest.Hit_timer2.win_condition.l1.date.month = GetAddingDataMonth(0, 0, 30);
			PChar.quest.Hit_timer2.win_condition.l1.date.year = GetAddingDataYear(0, 0, 30);
		...
		case "Hit_timer3":
			...
			PChar.quest.Hit_timer3.win_condition.l1.date.day = GetAddingDataDay(0, 0, 30);
			PChar.quest.Hit_timer3.win_condition.l1.date.month = GetAddingDataMonth(0, 0, 30);
			PChar.quest.Hit_timer3.win_condition.l1.date.year = GetAddingDataYear(0, 0, 30);
		...
		case "Hit_timer4":
			...
			PChar.quest.Hit_timer4.win_condition.l1.date.day = GetAddingDataDay(0, 0, 30);
			PChar.quest.Hit_timer4.win_condition.l1.date.month = GetAddingDataMonth(0, 0, 30);
			PChar.quest.Hit_timer4.win_condition.l1.date.year = GetAddingDataYear(0, 0, 30);
		...
		case "Hit_timer5":
			...
			PChar.quest.Hit_timer5.win_condition.l1.date.day = GetAddingDataDay(0, 0, 30);
			PChar.quest.Hit_timer5.win_condition.l1.date.month = GetAddingDataMonth(0, 0, 30);
			PChar.quest.Hit_timer5.win_condition.l1.date.year = GetAddingDataYear(0, 0, 30);
		...
		case "Hit_goto_QC_port":
			...
			pchar.quest.Hit_killed_Pepin.win_condition.l1 = "NPC_Death";
			pchar.quest.Hit_killed_Pepin.win_condition.l1.character = "Pepin Bertillon";
			pchar.quest.Hit_killed_Pepin.win_condition = "Hit_killed_Pepin";
		...
		// dialog exit from Amerigo
			...
			AddPartyExp(pchar, 5000);
			characters[GetCharacterIndex("Amerigo Vieira")].dialog.currentnode = "hired";
...
//Hitman
	...
	//Lucas
		...
		case "Lucas_ransom_start":
			...
			PChar.quest.Lucas_ransom_deadline.win_condition.l1.date.day = GetAddingDataDay(0, 0, 20);
			PChar.quest.Lucas_ransom_deadline.win_condition.l1.date.month = GetAddingDataMonth(0, 0, 30);
			PChar.quest.Lucas_ransom_deadline.win_condition.l1.date.year = GetAddingDataYear(0, 0, 30);
		...
		case "Lucas_ransom_delivery":
			if (!CheckQuestAttribute("gambling_with_girl", "to_FDF"))
			{
				StartQuestMovie(true, false, false);
				bDisableFastReload = 1;
			...
			{
				PChar.quest.Lucas_ransom_delivery_restore.win_condition.l1 = "location";
				PChar.quest.Lucas_ransom_delivery_restore.win_condition.l1.location = "Falaise_de_fleur_tavern";
				PChar.quest.Lucas_ransom_delivery_restore.win_condition = "Lucas_ransom_delivery";
			}
		break;
		...
		case "Lucas_join":
			EndQuestMovie();
			LAi_SetOfficerType(CharacterFromID("Lucas Da Saldanha"));
			AddPassenger(Pchar, characterFromID("Lucas Da Saldanha"), 0);
...
//Artois Voysey
		...
		case "Voysey_listed":
			...
			characters[GetCharacterIndex("Artois Voysey")].dialog.currentnode = "officer_Artois";
			pchar.quest.artois_died.win_condition.l1 = "NPC_Death";
			pchar.quest.artois_died.win_condition.l1.character = "Artois Voysey";
			pchar.quest.artois_died.win_condition = "artois_died";
		break;
		...
		case "Artois_missed":
			if (CheckQuestAttribute("generate_convoy_quest_progress", "") || !CheckAttribute(pchar, "quest.generate_convoy_quest_progress"))
			{
			...
			{
				LAi_QuestDelay("Artois_missed", 3.0);
			}
		break;
	...
	//Nigel Blythe
		...
		case "con_parri_prepare":
			...
			PChar.quest.con_parri_expired.win_condition.l1.date.day = GetAddingDataDay(0,0,30);
			...
			if (CheckQuestAttribute("generate_convoy_quest_progress", "") || !CheckAttribute(pchar, "quest.generate_convoy_quest_progress"))
			{
				if (!CharacterIsDead(&characters[GetCharacterIndex("Nigel Blythe")]))
				{
					...
					{	
						...
						RemovePassenger(pchar, characterFromID("Nigel Blythe"));
						LAi_SetSitType(CharacterFromID("Nigel Blythe"));
						ChangeCharacterAddressGroup(CharacterFromID("Nigel Blythe"), "Conceicao_tavern", "sit", "sit4");
					}
				}
			}
			else
			{
				LAi_QuestDelay("Nigel_Adventure_Conc", 3.0);
			}
		break;
		...
		case "Blythe_Battle":
			...
			{	
				...
				Pchar.quest.Battle2.win_condition.l1 = "Character_Capture";
				Pchar.quest.Battle2.win_condition.l1.character = "Pirates_7";
				PChar.quest.Battle2.win_condition = "Finish_battle";
			}
			else
			{
		...
		case "Smugglers_vs_Blythe":
			Group_SetAddress("Nigel Blythe", "none", "quest_ships", "quest_ship_2");
			pchar.quest.Battle1.over = "yes";
			bQuestDisableMapEnter = false;
			//RemoveCharacterCompanion(Pchar, characterFromID("Nigel Blythe"));
		...
		case "trade_quest_open":		
			...
			pchar.quest.generate_trade_quest.win_condition.l1.date.day = GetAddingDataDay(0, 0, 20);
    		pchar.quest.generate_trade_quest.win_condition.l1.date.month = GetAddingDataMonth(0, 0, 20);
			pchar.quest.generate_trade_quest.win_condition.l1.date.year = GetAddingDataYear(0, 0, 20);
			...
			switch (iTradeNation)
			{
				...
				case ENGLAND: 
					homelocation = pchar.quest.iTradeColony;
					switch (homelocation)
					{
						case "Redmond_store": DeleteQuestHeader("trade_red"); SetQuestHeader("trade_red"); AddQuestRecord("trade_red", "1"); pchar.colony = 0; break;
						case "Greenford_store": DeleteQuestHeader("trade_green"); SetQuestHeader("trade_green"); AddQuestRecord("trade_green", "1"); pchar.colony = 1; break;
						case "Oxbay_store": DeleteQuestHeader("trade_ox"); SetQuestHeader("trade_ox"); AddQuestRecord("trade_ox", "1"); pchar.colony = 2; break;
					}
				break;
			}	
		break;
		...
		case "close_trade_quest":
			...
			switch (iTradeNation)
			{
				...
				case ENGLAND: 
					...
					switch (homelocation)
					{
						case "Redmond_store": CloseQuestHeader("trade_red"); break;
						case "Greenford_store": CloseQuestHeader("trade_green"); break;
						case "Oxbay_store": CloseQuestHeader("trade_ox"); break;
					}
				break;
			}	
		break;
		...
		case "convoy_refused":
			...
			if (!checkquestattribute("generate_convoy_quest_progress", "begin"))
			{
				...
				switch (homelocation)
				{
					case "Falaise_de_fleur_port_01": CloseQuestHeader("convoy_fra"); break;
					case "Muelle_port": CloseQuestHeader("convoy_spa"); break;
					case "Douwesen_port": CloseQuestHeader("convoy_hol"); break;
					case "Conceicao_port": CloseQuestHeader("convoy_por"); break;
					case "Redmond_port": CloseQuestHeader("convoy_red"); break;
					case "Oxbay_port": CloseQuestHeader("convoy_ox"); break;
					case "Conceicao_shore_02": CloseQuestHeader("convoy_smg"); break;
				}	
			}
			pchar.quest.generate_convoy_quest_progress = "";
		break;
		...
		case "convoy_agreeded":
			...
			{
				case "Falaise_de_fleur_port_01": DeleteQuestHeader("convoy_fra"); SetQuestHeader("convoy_fra"); AddQuestRecord("convoy_fra", "1"); sDest = "Falaise_de_fleur_port_01"; break;
				case "Muelle_port": DeleteQuestHeader("convoy_spa"); SetQuestHeader("convoy_spa"); AddQuestRecord("convoy_spa", "1"); sDest = "Muelle_port"; break;
				case "Douwesen_port": DeleteQuestHeader("convoy_hol"); SetQuestHeader("convoy_hol"); AddQuestRecord("convoy_hol", "1"); sDest = "Douwesen_port"; break;
				case "Conceicao_port": DeleteQuestHeader("convoy_por"); SetQuestHeader("convoy_por"); AddQuestRecord("convoy_por", "1"); sDest = "Conceicao_port"; break;
				case "Redmond_port": DeleteQuestHeader("convoy_red"); SetQuestHeader("convoy_red"); AddQuestRecord("convoy_red", "1"); sDest = "Redmond_port"; break;
		...
		case "prepare_for_meet_ferro":
			...
			PChar.quest.ferro_to_conceicao.win_condition.l1.date.day = GetAddingDataDay(0, 0, 1);
			PChar.quest.ferro_to_conceicao.win_condition.l1.date.month = GetAddingDataMonth(0, 0, 1);
			PChar.quest.ferro_to_conceicao.win_condition.l1.date.year = GetAddingDataYear(0, 0, 1);
		...
		case "baldewyn_wait_month":
			...
			PChar.quest.baldewyn_wait_month_timer.win_condition.l1.date.day = GetAddingDataDay(0, 0, 30);
			PChar.quest.baldewyn_wait_month_timer.win_condition.l1.date.month = GetAddingDataMonth(0, 0, 30);
			PChar.quest.baldewyn_wait_month_timer.win_condition.l1.date.year = GetAddingDataYear(0, 0, 30);
		...
		case "kill_pirate_agree":
			...
			pchar.quest.prepare_for_kill_pirate.win_condition.l1.location = pchar.quest.killdestination;
			pchar.quest.prepare_for_kill_pirate.win_condition = "prepare_for_kill_pirate";
			//завод книги
			switch (pchar.quest.killdestination)
			{
				case "Falaisedefleur": DeleteQuestHeader("kill_fal"); SetQuestHeader("kill_fal"); AddQuestRecord("kill_fal", "1"); break;
				case "IslaMuelle": DeleteQuestHeader("kill_mue"); SetQuestHeader("kill_mue"); AddQuestRecord("kill_mue", "1"); break;
				case "Douwesen": DeleteQuestHeader("kill_dou"); SetQuestHeader("kill_dou"); AddQuestRecord("kill_dou", "1"); break;
				case "Conceicao": DeleteQuestHeader("kill_con"); SetQuestHeader("kill_con"); AddQuestRecord("kill_con", "1"); break;
				case "Redmond": DeleteQuestHeader("kill_red"); SetQuestHeader("kill_red"); AddQuestRecord("kill_red", "1"); break;
				case "Oxbay": DeleteQuestHeader("kill_ox"); SetQuestHeader("kill_ox"); AddQuestRecord("kill_ox", "1"); break;
			}			
		break;
		
		case "prepare_for_kill_pirate":
			characters[GetCharacterIndex("Quest Pirate")].skill.accuracy = pchar.skill.accuracy;
			characters[GetCharacterIndex("Quest Pirate")].skill.cannons = pchar.skill.cannons;
			characters[GetCharacterIndex("Quest Pirate")].skill.fencing = pchar.skill.fencing;
			...
			pchar.quest.capture_pirate_complete.win_condition.l1 = "Character_Capture";
			pchar.quest.capture_pirate_complete.win_condition.l1.character = "Quest pirate";
			pchar.quest.capture_pirate_complete.win_condition = "kill_pirate_complete";
		break;
		
		case "kill_pirate_complete":
			pchar.quest.capture_pirate_complete.over = "yes";
			pchar.quest.kill_pirate_complete.over = "yes";
		...
		case "kill_pirate_refused":
			...
			pchar.quest.quest.generate_kill_quest = "refused";
		break;
		...
		case "kill_pirate_refused_2":		
			...
			switch (pchar.quest.killdestination)
			{
				case "Falaisedefleur": CloseQuestHeader("kill_fal"); break;
				case "IslaMuelle": CloseQuestHeader("kill_mue"); break;
				case "Douwesen": CloseQuestHeader("kill_dou"); break;
				case "Conceicao": CloseQuestHeader("kill_con"); break;
				case "Redmond": CloseQuestHeader("kill_red"); break;
				case "Oxbay": CloseQuestHeader("kill_ox"); break;
			}
		break;
		...
		case "to_greenford_for_teacher":
			LAi_LoginInCaptureTown(characterFromID("Greenford Prison Commendant"), true);
			...
			LAi_SetHuberType(characterFromID("Greenford Prison Commendant"));
			LAi_removeLoginTime(characterFromID("Greenford Prison Commendant"));
			LAi_SetHp(characterFromID("Greenford Prison Commendant"), 80.0, 80.0);
		break;
		
		//линейка контрабандистов
		case "smuggler_line_begin":
			pchar.quest.smuggler_line = "begin";
			PlaceCharacter(characterFromID("quest_smuggler_02"), "goto", "redmond_town_01");
			LAi_SetActorType(characterFromID("quest_smuggler_02"));
			Lai_ActorDialog(characterFromID("quest_smuggler_02"), pchar, "", 3.0, 1.0);
		break;
		
		case "smuggler_line_begin_exit":
			homelocator = LAi_FindFarLocator("reload", locx, locy, locz);
			...
			characters[GetCharacterIndex("Camilo Machado")].dialog.currentnode = "smuggler_line";
			pchar.quest.smuggler_line = "to_machado";
		break;
		...
		case "smuggler_line_first_mission_wait_night":
		...
		case "smuggler_line_first_mission_before_soldiers":
			...
			LAi_SetActorType(characterFromID("Arnaud Matton"));
			LAi_ActorDialogNow(characterFromID("Arnaud Matton"), pchar, "", -1);
			characters[GetCharacterIndex("Arnaud Matton")].dialog.curentnode = "smuggler_line_first_mission_before_soldiers";
		break;
		...
		case "smuggler_line_first_mission_soldiers":
			...
			sld = LAi_CreateFantomCharacter("Offic_fra", "reload", "reload1");
			LAi_group_MoveCharacter(sld, "FRANCE_SOLDIERS");
		...
		case "smuggler_line_first_mission_arnaud_first_talk_with_soldiers":
			pchar.quest.smuggler_line = "smuggler_line_first_mission_arnaud_first_talk_with_soldiers";
			characters[GetCharacterIndex("Arnaud Matton")].dialog.currentnode = "smuggler_line_first_mission_arnaud_first_talk_with_soldiers";
			LAi_ActorDialogNow(characterFromID("Arnaud Matton"), pchar, "", -1.0);
		break;
		
		case "smuggler_line_first_mission_soldiers_arrest_matton":
			pchar.quest.smuggler_line = "smuggler_line_first_mission_soldiers_arrest_matton";
			...
			LAi_ActorFollow(characterFromID("smuggler_line_soldier_01"), characterFromID("Arnaud Matton"), "", 15.0);
			LAi_ActorFollow(characterFromID("smuggler_line_soldier_02"), characterFromID("Arnaud Matton"), "", 15.0);
			LAi_ActorFollow(characterFromID("smuggler_line_soldier_03"), characterFromID("Arnaud Matton"), "", 15.0);
			
			pchar.quest.smuggler_line_first_mission_soldiers_arrest_matton_exit.win_condition.l1 = "Location";
			pchar.quest.smuggler_line_first_mission_soldiers_arrest_matton_exit.win_condition.l1.location = "Falaise_de_fleur_location_04";
			pchar.quest.smuggler_line_first_mission_soldiers_arrest_matton_exit.win_condition = "smuggler_line_first_mission_soldiers_arrest_matton_exit";
		break;
		
		case "smuggler_line_first_mission_soldiers_arrest_matton_exit":			
			Locations[FindLocation("Falaise_de_fleur_location_04")].reload.l3.disable = 1;
			LAi_ActorSelfDialog(pchar, "");
			AddQuestRecord("smuggler_line", "6");
			
			pchar.quest.smuggler_line_first_mission_wait_for_matton.win_condition.l1 = "Timer";
			pchar.quest.smuggler_line_first_mission_wait_for_matton.win_condition.l1.date.day = GetAddingDataDay(0, 0, 1);
   			pchar.quest.smuggler_line_first_mission_wait_for_matton.win_condition.l1.date.month = GetAddingDataMonth(0, 0, 1);
			pchar.quest.smuggler_line_first_mission_wait_for_matton.win_condition.l1.date.year = GetAddingDataYear(0, 0, 1);
			pchar.quest.smuggler_line_first_mission_wait_for_matton.win_condition = "smuggler_line_first_mission_wait_for_matton";
			
			pchar.quest.smuggler_line_first_mission_wait_for_matton_decline.win_condition.l1 = "Location";
			pchar.quest.smuggler_line_first_mission_wait_for_matton_decline.win_condition.l1.location = "FalaiseDeFleur";
			pchar.quest.smuggler_line_first_mission_wait_for_matton_decline.win_condition = "smuggler_line_first_mission_wait_for_matton_decline";
		break;
		
		case "smuggler_line_first_mission_wait_for_matton":
			AddQuestRecord("smuggler_line", "7");
		break;
		
		case "smuggler_line_first_mission_wait_for_matton_decline":
			pchar.quest.smuggler_line_first_mission_wait_for_matton.over = "yes";
		break;
		...
		case "smuggler_line_first_mission_award":
			...
			LAi_SetStayType(characterFromID("Arnaud Matton"));
		...
		case "second_mission_money_prepare_for_troubles":
			GiveItem2Character(Pchar, "Arnaud_ring");
			...
			pchar.quest.second_mission_money_prepare_for_troubles_port.win_condition.l1.location = pchar.location.from_sea;
		...
		case "second_mission_money_troubles":
			GetCharacterPos(GetMainCharacter(), &locx, &locy, &locz);
			homelocator = LAi_FindNearestFreeLocator("goto", locx, locy, locz);
			sld = LAi_CreateFantomCharacter("Offic_fra", "goto", homelocator);
			LAi_group_MoveCharacter(sld, "FRANCE_SOLDIERS");
			sld.id = "smuggler_line_soldier_03";
			sld.dialog.filename = "quest_soldier_dialog.c";
			LAi_setActorType(sld);
			LAi_ActorDialog(sld, pchar, "", 3.0, 1.0);
			LAi_SetAdjustFencingSkill(sld, 1.0, 2.0);
			
			GetCharacterPos(GetMainCharacter(), &locx, &locy, &locz);
			homelocator = LAi_FindNearestFreeLocator("goto", locx, locy, locz);
			sld = LAi_CreateFantomCharacter("Soldier_fra", "goto", homelocator);
			LAi_group_MoveCharacter(sld, "FRANCE_SOLDIERS");
			sld.id = "smuggler_line_soldier_01";
			LAi_setActorType(sld);
			LAi_ActorFollow(sld, pchar, "", 2.0);
			...
			GetCharacterPos(GetMainCharacter(), &locx, &locy, &locz);
			homelocator = LAi_FindNearestFreeLocator("goto", locx, locy, locz);
			sld = LAi_CreateFantomCharacter("Soldier_fra2", "goto", homelocator);
			LAi_group_MoveCharacter(sld, "FRANCE_SOLDIERS");
			sld.id = "smuggler_line_soldier_02";
			LAi_setActorType(sld);
			LAi_ActorFollow(sld, pchar, "", 2.0);
		...
		case "second_mission_money_troubles_fight":
			...
			pchar.quest.smuggler_line = "second_mission_to_machado_with_money_and_with_ring";
		break;
		
		case "second_mission_money_to_france_governor":
			AddQuestRecord("smuggler_line", "10");
			AddQuestRecord("smuggler_line", "12");
			DoQuestReloadToLocation("Falaise_de_fleur_townhall", "goto", "goto7", "second_mission_speak_with_france_governor");
			RemoveCharacterEquip(pchar, BLADE_ITEM_TYPE);
			RemoveCharacterEquip(pchar, GUN_ITEM_TYPE);
			EnableEquip(pchar, BLADE_ITEM_TYPE, false);
			EnableEquip(pchar, GUN_ITEM_TYPE, false);
		...
		case "second_mission_speak_with_france_governor":
			...
			sld = LAi_CreateFantomCharacter("Soldier_fra", "goto", "goto9");
			LAi_group_MoveCharacter(sld, "FRANCE_SOLDIERS");
			sld.id = "smuggler_line_soldier_01";
			LAi_setActorType(sld);
			LAi_ActorFollow(sld, pchar, "", 2.0);
			...
			sld = LAi_CreateFantomCharacter("Soldier_fra2", "goto", "goto6");
			LAi_group_MoveCharacter(sld, "FRANCE_SOLDIERS");
			sld.id = "smuggler_line_soldier_02";
			LAi_setActorType(sld);
			LAi_ActorFollow(sld, pchar, "", 2.0);
			...
			sld = LAi_CreateFantomCharacter("Offic_fra", "goto", "goto4");
			LAi_group_MoveCharacter(sld, "FRANCE_SOLDIERS");
			sld.id = "smuggler_line_soldier_03";
			sld.dialog.filename = "quest_soldier_dialog.c";
			LAi_setActorType(sld);
			...
			characters[getCharacterIndex("Joseph Claude Le Moigne")].dialog.currentnode = "smuggler_line";
			LAi_ActorWaitDialog(Pchar, characterFromID("Joseph Claude Le Moigne"));
			LAi_ActorDialog(characterFromID("Joseph Claude Le Moigne"), pchar, "", 3.0, 1.0);
		break;
		
		case "second_mission_money_delivered_to_machado":
		...
		case "second_mission_to_prison":
			characters[getCharacterIndex("Joseph Claude Le Moigne")].dialog.currentnode = "second time";
			LAi_SetHuberStayType(characterFromID("Joseph Claude Le Moigne"));
			SetTownCapturedState("Falaise de fleur", true);
			StorePassengers("Blaze");
			DoQuestReloadToLocation("FDF_prison", "goto", "goto9", "second_mission_prison_wait_for_resque");
		break;
		...
		case "second_mission_prison_prepare_for_resque":
			pchar.quest.smuggler_line = "in_fdf_prison";
			AddQuestRecord("smuggler_line", "13");
			LAi_SetPlayerType(pchar);
			ChangeCharacterAddressGroup(characterFromID("danielle"), "FDF_prison", "merchant", "reload13");
			LAi_SetActorType(characterFromID("danielle"));
		...
		case "second_mission_exit_from_cam_completed":
			...
			EquipCharacterByItem(pchar, FindCharacterItemByGroup(pchar, BLADE_ITEM_TYPE));
			EquipCharacterByItem(pchar, FindCharacterItemByGroup(pchar, GUN_ITEM_TYPE));
			ChangeCharacterAddress(pchar, "FDF_prison", "goto12");
			pchar.quest.smuggler_line = "second_mission_exit_from_prison";
			ChangeCharacterAddress(characterFromID("danielle"), "FDF_prison", "goto23");
			if (CheckQuestAttribute("danielle_on_ship", "true"))
			{
				...
				RestorePassengers("Blaze");
				LAi_SetOfficerType(characterFromID("danielle"));
				pchar.quest.smuggler_line = "second_mission_return_to_machado_after_prison";
			...
			reStorePassengers("Blaze");
			pchar.quest.smuggler_line_second_mission_uncapture_fdf.win_condition.l1 = "Timer";
			pchar.quest.smuggler_line_second_mission_uncapture_fdf.win_condition.l1.date.day = GetAddingDataDay(0, 0, 6);
    			pchar.quest.smuggler_line_second_mission_uncapture_fdf.win_condition.l1.date.month = GetAddingDataMonth(0, 0, 6);
			pchar.quest.smuggler_line_second_mission_uncapture_fdf.win_condition.l1.date.year = GetAddingDataYear(0, 0, 6);
			pchar.quest.smuggler_line_second_mission_uncapture_fdf.win_condition = "smuggler_line_second_mission_uncapture_fdf";
		break;
		...
		case "smuggler_line_second_mission_uncapture_fdf":
			SetTownCapturedState("Falaise de Fleur", false);
		break;
		
		case "second_mission_money_not_delivered_to_machado":
		...
		case "second_mission_wait_two_days":
			...
			pchar.quest.second_mission_to_tavern_room.win_condition.l1.location = "Falaise_de_fleur_tavern_upstairs";
		...
		case "second_mission_meet_with_killers":
			...
			sld = LAi_CreateFantomCharacter("mask_2", "goto", "goto9");
			sld.id = "smuggler_line_killer_01";
			LAi_setActorType(sld);
			LAi_ActorFollow(sld, pchar, "", 2.0);
			LAi_SetAdjustFencingSkill(sld, 1.0, 2.0);
			
			sld = LAi_CreateFantomCharacter("mask_3", "goto", "goto6");
		...
		case "second_mission_after_fight_with_killers_return_to_machado":
			AddQuestRecord("smuggler_line", "20");
			AddPartyExp(pchar, 1500);
		break;
		
		case "second_mission_truth_to_matton":
			AddQuestRecord("smuggler_line", "21");
			pchar.quest.smuggler_line = "second_mission_truth_to_matton";
		break;
		
		case "second_mission_truth_to_matton_and_say_machado":
		...
		case "third_mission_accept":
			...
			pchar.quest.machado_death.win_condition.l1 = "Npc_death";
			pchar.quest.machado_death.win_condition.l1.character = "Camilo Machado";
			pchar.quest.machado_death.win_condition = "machado_death";
		break;
		...
		case "machado_death":
			...
			if (checkQuestAttribute("smuggler_line", "third_mission_to_douwesen_expired_fire_fight") || checkQuestAttribute("smuggler_line", "fourth_mission_kill_arnaud_fire_fight"))
			{
				AddQuestRecord("smuggler_line", "29");
				ChangeCharacterReputation (pchar, -10);
				GiveItem2Character(pchar, "blade24");
				Log_SetStringToLog("С трупа Камило Мачадо вы взяли клинок, на лезвии которого стоит клеймо - 'УРАГАН'.");
			}
			if (checkQuestAttribute("smuggler_line", "traitor"))
			{
				...
				Group_CreateGroup("Machado");
				Group_AddCharacter("Machado", "Camilo Machado");
				Group_SetGroupCommander("Machado", "Camilo Machado");
				Group_SetAddress("Machado", "Conceiaco", "Quest_Ships","Quest_Ship_7");
				SetCharacterRelationBoth(GetCharacterIndex("Camilo Machado"),GetMainCharacterIndex(),RELATION_ENEMY);
				LAi_LocationFightDisable(&locations[FindLocation(pchar.location)], true);
			}
			...
			if (checkQuestAttribute("smuggler_line", "traitor3"))
			{
				...
				Group_AddCharacter("Machado", "Camilo Machado");
				Group_SetGroupCommander("Machado", "Camilo Machado");
				Group_SetAddress("Machado", "Douwesen", "Quest_Ships","Quest_Ship_7");
				SetCharacterRelationBoth(GetCharacterIndex("Camilo Machado"),GetMainCharacterIndex(),RELATION_ENEMY);
				LAi_ActorDialog(characterFromID("Isenbrandt Jurcksen"), pchar, "", 2.0, 1.0);
		...
		case "third_mission_to_douwesen":
			...
			Group_SetGroupCommander("Machado", "Camilo Machado");
			Group_SetAddress("Machado", "Douwesen", "Quest_Ships","Quest_Ship_7");
			SetCharacterRelationBoth(GetCharacterIndex("Camilo Machado"),GetMainCharacterIndex(),RELATION_FRIEND);
		break;
		...
		case "third_mission_to_douwesen_expired_fire_fight":
			...
			LAi_SetWarriorType(characterFromID("Camilo Machado"));
			LAi_group_MoveCharacter(characterFromID("Camilo Machado"), "SMUGGLERS_SOLDIERS");
			LAi_group_SetRelation("SMUGGLERS_SOLDIERS", LAI_GROUP_PLAYER, LAI_GROUP_ENEMY);
			LAi_group_FightGroups("SMUGGLERS_SOLDIERS", LAI_GROUP_PLAYER, true);
		break;
		...
		case "third_mission_to_douwesen_shore":
			...
			Locations[FindLocation("Douwesen_shore_02")].reload.l2.disable = 1;
			ChangeCharacterAddress(characterFromID("Camilo Machado"), "Douwesen_shore_02", "goto7");
			ChangeCharacterAddress(characterFromID("Isenbrandt Jurcksen"), "Douwesen_shore_02", "goto5");
			LAi_SetActorType(characterFromID("Camilo Machado"));
			LAi_ActorDialog(characterFromID("Camilo Machado"), pchar, "", 4.0, 1.0);
		break;
		
		case "third_mission_speak_with_pirate_head":
			characters[GetCharacterIndex("Isenbrandt Jurcksen")].dialog.smgnode = characters[GetCharacterIndex("Isenbrandt Jurcksen")].dialog.currentnode;
			characters[GetCharacterIndex("Isenbrandt Jurcksen")].dialog.currentnode = "third time two";
			pchar.quest.smuggler_line = "third_mission_speak_with_pirate_head";
			LAi_ActorFollow(characterFromID("Camilo Machado"), characterFromID("Isenbrandt Jurcksen"), "", 10.0);
		...
		case "third_mission_pirate_head_exit_from_shore":
			...
			LAi_ActorRunToLocation(characterFromID("Isenbrandt Jurcksen"), "reload", "reload2_back", "QC_residence", "goto", "goto1", "third_mission_pirate_head_exit_from_shore_enter_to_residence", 25.0);
			LAi_type_actor_Reset(characterFromID("Camilo Machado"));
			LAi_ActorDialog(characterFromID("Camilo Machado"), pchar, "", 1.0, 1.0);
			Locations[FindLocation("Douwesen_shore_02")].reload.l2.disable = 0;
		break;
		
		case "third_mission_pirate_head_exit_from_shore_enter_to_residence":
			LAi_SetStayType(characterFromID("Isenbrandt Jurcksen"));
			characters[GetCharacterIndex("Isenbrandt Jurcksen")].dialog.currentnode = characters[GetCharacterIndex("Isenbrandt Jurcksen")].dialog.smgnode;
		break;
		...
		case "fourth_mission_acepted":
			...
			pchar.quest.fourth_mission_to_fdf.win_condition.l1 = "Location";
			pchar.quest.fourth_mission_to_fdf.win_condition.l1.location = "Falaise_de_fleur_location_04";
			pchar.quest.fourth_mission_to_fdf.win_condition = "fourth_mission_to_fdf";
			characters[GetCharacterIndex("Camilo Machado")].location = "Smugglers_Residence";
			characters[GetCharacterIndex("Camilo Machado")].location.group = "goto";
			characters[GetCharacterIndex("Camilo Machado")].location.locator = "goto1";
			Group_SetAddress("Machado", "none", "Quest_Ships","Quest_Ship_7");
			LAi_SetStayType(characterFromID("Camilo Machado"));
		break;
		
		case "fourth_mission_to_fdf":
			pchar.quest.smuggler_line = "fourth_mission_to_fdf";
			characters[GetCharacterIndex("Sabine Matton")].dialog.smgnode = characters[GetCharacterIndex("Sabine Matton")].dialog.currentnode;
			characters[GetCharacterIndex("Sabine Matton")].dialog.currentnode = "smuggler_line";
			PlaceCharacter(characterFromID("Sabine Matton"), "goto", "Falaise_de_fleur_location_04");
			LAi_SetActorType(characterFromID("Sabine Matton"));
			LAi_ActorDialog(characterFromID("Sabine Matton"), pchar, "", 2.0, 1.0);
		break;
		
		case "fourth_mission_capture_sabine":
			pchar.quest.smuggler_line = "fourth_mission_capture_sabine";
			LAi_ActorRunToLocation(characterFromID("sabine matton"), "reload", "Falaise_de_fleur_location_04_05", "none", "", "", "", 25.0);
			AddPassenger(Pchar, characterFromID("sabine matton"), 1);
			AddQuestRecord("smuggler_line", "32");
		break;
		
		case "fourth_mission_capture_sabine_wait_in_tavern":
			AddPartyExp(pchar, 2000);
			pchar.quest.smuggler_line = "fourth_mission_capture_sabine_wait_in_tavern";
			AddQuestRecord("smuggler_line", "33");
			pchar.quest.fourth_mission_to_tavern.win_condition.l1 = "location";
			pchar.quest.fourth_mission_to_tavern.win_condition.l1.location = "Falaise_de_fleur_tavern_upstairs";
			pchar.quest.fourth_mission_to_tavern.win_condition = "fourth_mission_to_tavern";
		break;
		
		case "fourth_mission_to_tavern":
			LAi_QuestDelay("fourth_mission_to_tavern_wait_arnaud", 1.0);
		break;
		
		case "fourth_mission_to_tavern_wait_arnaud":
			Lai_setActorType(pchar);
			Lai_ActorGoToLocator(pchar, "goto", "goto4", "fourth_mission_meet_with_arnaud_in_tavern", 5.0);
		break;
		
		case "fourth_mission_meet_with_arnaud_in_tavern":
			LAi_RemoveLoginTime(characterFromID("Arnaud Matton"));
			characters[GetCharacterIndex("Arnaud Matton")].dialog.currentnode = "resque_sabine";
			pchar.quest.smuggler_line = "fourth_mission_meet_with_arnaud_in_tavern";
			Locations[FindLocation("Falaise_de_fleur_location_04")].reload.l3.disable = 1;
			ChangeCharacterAddressGroup(characterFromID("Arnaud Matton"), "Falaise_de_fleur_tavern_upstairs", "reload", "reload1");
			LAi_SetActorType(characterFromID("Arnaud matton"));
			LAi_ActorWaitDialog(Pchar, characterFromID("Arnaud Matton"));
			LAi_ActorDialog(characterFromID("Arnaud Matton"), pchar, "", 3.0, 1.0);
		break;
		
		case "fourth_mission_meet_with_arnaud_in_tavern_good":
			LAi_ActorRunToLocation(characterFromID("arnaud matton"), "reload", "reload1", "none", "", "", "", 25.0);
			RemovePassenger(Pchar, characterFromID("Sabine Matton"));
			GiveItem2Character(Pchar, "shop_papers");
			pchar.quest.smuggler_line = "fourth_mission_meet_with_arnaud_in_tavern_good";
		...
		case "fourth_mission_meet_with_arnaud_in_tavern_bad":
			...
			pchar.quest.fourth_mission_soldiers.win_condition.l1.location = "Falaise_de_Fleur_location_03";
		...
		case "fourth_mission_fight":
			LAi_LocationFightDisable(&Locations[FindLocation(pchar.location)], false);
			LAi_SetHp(characterFromID("Arnaud Matton"), 1.0, 1.0);
			LAi_SetActorType(pchar);
			LAi_ActorAttack(pchar, characterFromID("Arnaud Matton"), "fourth_mission_kill_arnaud");
		break;
		
		case "fourth_mission_kill_arnaud":
			LAi_LocationFightDisable(&Locations[FindLocation(pchar.location)], true);
			...
			pchar.quest.fourth_mission_soldiers.win_condition.l1.location = "Falaise_de_Fleur_location_03";
			pchar.quest.fourth_mission_soldiers.win_condition = "fourth_mission_soldiers";
			pchar.quest.smuggler_line = "fourth_mission_kill_arnaud";
		break;
		
		case "fourth_mission_completed":
			pchar.quest.smuggle_line.fourth_mission = "completed";
			...
			characters[GetCharacterIndex("arnaud matton")].location.group = "goto";
			characters[GetCharacterIndex("arnaud matton")].location.locator = "goto1";
			Lai_SetStayType(characterFromID("arnaud matton"));
		break;
		
		case "fourth_mission_kill_arnaud_fire_fight":
			pchar.quest.smuggler_line = "fourth_mission_kill_arnaud_fire_fight";
			LAi_SetWarriorType(characterFromID("Camilo Machado"));
			LAi_group_MoveCharacter(characterFromID("Camilo Machado"), "SMUGGLERS_SOLDIERS");
			LAi_group_SetRelation("SMUGGLERS_SOLDIERS", LAI_GROUP_PLAYER, LAI_GROUP_ENEMY);
			LAi_group_FightGroups("SMUGGLERS_SOLDIERS", LAI_GROUP_PLAYER, true);
		...
		case "fourth_mission_kill_arnaud_fire_without_fight":
			AddQuestRecord("smuggler_line", "28");
			pchar.quest.smuggler_line = "failed";
			CloseQuestHeader("smuggler_line");
			Locations[FindLocation("Falaise_de_fleur_location_04")].reload.l3.disable = 0;
		...
		case "fourth_mission_failed":
			...
			characters[GetCharacterIndex("arnaud matton")].location.locator = "goto1";
			Lai_SetStayType(characterFromID("arnaud matton"));
			LAi_CharacterEnableDialog(characterFromID("arnaud matton"));
			LAi_CharacterEnableDialog(characterFromID("hardouin aufort"));
			ChangeCharacterAddress(characterFromID("hardouin aufort"), "falaise_de_fleur_location_03", "soldier04");
			LAi_SetPatrolType(characterFromID("hardouin aufort"));
		break;
		
		case "fourth_mission_to_arnaud":
			pchar.quest.smuggler_line = "fourth_mission_to_arnaud";
		break;
		...
		case "fourth_mission_with_officer_to_shop_wrong":
			...
			LAi_ActorRunToLocation(characterFromID("hardouin Aufort"), "reload", "Falaise_de_Fleur_location_03_04", "none", "", "", "", 95.0);
			pchar.quest.fourth_mission_with_officer_to_shop_complete.win_condition.l1 = "location";
			pchar.quest.fourth_mission_with_officer_to_shop_complete.win_condition.l1.location = "Falaise_de_Fleur_store";
		...
		case "fourth_mission_with_officer_to_shop_complete":
			LAi_type_actor_Reset(characterFromID("hardouin Aufort"));
			PlaceCharacter(characterFromID("hardouin Aufort"), "goto", pchar.location);
			Lai_ActorDialog(characterFromID("hardouin Aufort"), pchar, "", 2.0, 1.0);
		break;
		...
		case "fourth_mission_after_shop_wrong":
			...
			pchar.quest.smuggler_line = "fourth_mission_with_officer_to_shop_wrong";
			Locations[FindLocation("Falaise_de_fleur_location_04")].reload.l3.disable = 1;
			LAi_CharacterDisableDialog(characterFromID("arnaud matton"));
			LAi_CharacterDisableDialog(characterFromID("hardouin aufort"));
		break;
		
		case "fourth_mission_officer_exit_from_shop":
			LAi_ActorRunToLocation(characterFromID("hardouin Aufort"), "reload", "reload1", "none", "falaise_de_fleur_location_03", "goto", "soldier04", 5.0);
		...
		case "fourth_mission_return_to_machado":
			pchar.quest.smuggler_line = "fourth_mission_return_to_machado";
		...
		case "fifth_mission_trap_begin":
			...
			Group_AddCharacter("Story_AntiSmuggler_Squadron2", "Smuggler_quest_pirate_03");
			Group_AddCharacter("Story_AntiSmuggler_Squadron2", "Smuggler_quest_pirate_04");
			...
			if (!CheckQuestAttribute("smuggler_barrakuda", "died"))
			{
				...
				RemoveCharacterCompanion(Pchar, characterFromID("Barrakuda"));
				AddQuestRecord("smuggler_line", "57");
				Log_SetStringToLog("Барракуда покинул вас и принял участие в битве на стороне противника.");
			...
			Group_SetTaskAttack("Story_AntiSmuggler_Squadron", PLAYER_GROUP);
			...
			Group_SetTaskAttack("Story_AntiSmuggler_Squadron", "Story_Smuggler_Squadron");
			Group_LockTask("Story_AntiSmuggler_Squadron");
			
			Group_SetTaskAttack("Story_Smuggler_Squadron", "Story_AntiSmuggler_Squadron2");
		...
		case "barrakuda_died":
			...
			{
				if (pchar.ship.type == SHIP_BARRAKUDA || characters[GetCompanionIndex(pchar, 1)] == SHIP_BARRAKUDA || characters[GetCompanionIndex(pchar, 2)] == SHIP_BARRAKUDA || characters[GetCompanionIndex(pchar, 3)] == SHIP_BARRAKUDA)
				{
		...
		case "pirate_line_to_black_pearl":
			pchar.quest.pirate_line = "pirate_line_to_black_pearl";
			LAi_ActorRunToLocation(characterFromID("Isenbrandt Jurcksen"), "reload", "reload2_back", "QC_residence", "goto", "goto1", "third_mission_pirate_head_exit_from_shore_enter_to_residence", 25.0);
			SetQuestheader("pirate_line");
			AddQuestRecord("pirate_line", "1");
			AddPassenger(Pchar, characterFromID("orlando"), 0);
			
			pchar.quest.pirate_line_to_black_pearl_complete.win_condition.l1 = "location";
			pchar.quest.pirate_line_to_black_pearl_complete.win_condition.l1.location = "Khael Roa";
			pchar.quest.pirate_line_to_black_pearl_complete.win_condition = "pirate_line_to_black_pearl_complete";
			
			aref refShip;
			ref  refBaseShip;
			makearef(refShip,characters[getCharacterIndex("Barbossa")].Ship);
			refBaseShip = GetShipByType(SHIP_BLACK_PEARL);
			refShip.HP = 20000;
			refBaseShip.HP = 20000;
		break;
		
		case "pirate_line_to_black_pearl_complete":
			...
			pchar.quest.capture_black_pearl.win_condition.l1= "character_capture";
			pchar.quest.capture_black_pearl.win_condition.l1.character = "Barbossa";
			pchar.quest.capture_black_pearl.win_condition = "capture_black_pearl";

			pchar.quest.sink_black_pearl.win_condition.l1 = "NPC_Death";
			pchar.quest.sink_black_pearl.win_condition.l1.character = "Barbossa";
			pchar.quest.sink_black_pearl.win_condition = "sink_black_pearl";
		break;
		
		case "capture_black_pearl":
			pchar.quest.pirate_line = "capture_black_pearl";
			AddQuestRecord("pirate_line", "2");
			Group_SetAddress("Black_Pearl", "none", "Quest_Ships","Quest_Ship_2");
		break;
		
		case "sink_black_pearl":
			if(!CheckQuestAttribute("pirate_line", "capture_black_pearl"))
			{
				pchar.quest.pirate_line = "sink_black_pearl";
				AddQuestRecord("pirate_line", "3");
				Group_SetAddress("Black_Pearl", "none", "Quest_Ships","Quest_Ship_2");
			}
		break;
		...
		case "sink_black_pearl_fight":
			...
			LAi_group_MoveCharacter(characterFromID("Isenbrandt Jurcksen"), "QC_SOLDIERS");
			...
			pchar.quest.jurcksen_death.win_condition.l1 = "Npc_death";
			pchar.quest.jurcksen_death.win_condition.l1.character = "Isenbrandt Jurcksen";
			pchar.quest.jurcksen_death.win_condition = "jurcksen_death";			
		break;
		
		case "jurcksen_death":
			AddQuestRecord("pirate_line", "4");
			CloseQuestHeader("pirate_line");
			ChangeCharacterReputation (pchar, -10);
			GiveItem2Character(pchar, "blade25");
			Log_SetStringToLog("С трупа главаря пиратов вы взяли клинок, на лезвии которого стоит клеймо - 'МЕЧТА ПИРАТА'.");
			
			Group_CreateGroup("jurcksen");
			Group_AddCharacter("jurcksen", "Isenbrandt Jurcksen");
			Group_SetGroupCommander("jurcksen", "Isenbrandt Jurcksen");
			Group_SetAddress("jurcksen", "ЙС", "Quest_Ships","Quest_Ship_1");
			SetCharacterRelationBoth(GetCharacterIndex("Isenbrandt Jurcksen"),GetMainCharacterIndex(),RELATION_ENEMY);
			LAi_LocationFightDisable(&locations[FindLocation(pchar.location)], true);
		break;
		
		case "sink_black_pearl_no_fight":
			AddQuestRecord("pirate_line", "5");
			CloseQuestHeader("pirate_line");
		break;
		
		case "capture_black_pearl_no_fight":
			if (pchar.ship.type == SHIP_BLACK_PEARL)
			{
				pchar.ship.type = SHIP_PROPHECY;
			}
			if(characters[GetCompanionIndex(pchar, 1)] == SHIP_BLACK_PEARL)
			{
				characters[GetCompanionIndex(pchar, 1)].ship.type = SHIP_PROPHECY;
			}
			if(characters[GetCompanionIndex(pchar, 2)] == SHIP_BLACK_PEARL)
			{
				characters[GetCompanionIndex(pchar, 2)].ship.type = SHIP_PROPHECY;
			}
			if(characters[GetCompanionIndex(pchar, 3)] == SHIP_BLACK_PEARL)
			{
				characters[GetCompanionIndex(pchar, 3)].ship.type = SHIP_PROPHECY;
			}
			AddQuestRecord("pirate_line", "6");
			CloseQuestHeader("pirate_line");
			pchar.quest.pirate_line = "completed";
			AddPartyExp(pchar, 20000);
			AddMoneyToCharacter(pchar, 20000);
		break;
	
		//Amanda
		case "AmandaQuest":		//начало скрипта
			ChangeCharacterAddress(characterFromID("Amanda"), "Conceicao_port", "goto8");
			LAi_SetActorType(characterFromID("Amanda"));
			LAi_ActorDialog(characterFromID("Amanda"), PChar, "", 3.0, 1.0);
			break;

		case "AmandaSit":
			AddPassenger(pchar, CharacterFromID("Amanda"), 0);
			SetCharacterRemovable(characterFromID("Amanda"), false);
			OfficersReaction("good");
			PlaceCharacter(characterFromID("Amanda"), "goto", "");

			//pchar.quest.AmandaQuest.win_condition = "AmandaInDou";
			break;

		case "AmandaInDou":
			ChangeCharacterAddress(characterFromID("Amanda"), "Douwesen_port", "goto18");
			LAi_SetCitizenType(characterFromID("Amanda"));
			Characters[GetCharacterIndex("Amanda")].Dialog.CurrentNode = "next4";
			break;

		case "AmandaLeaveDou":
			LAi_SetCitizenType(characterFromID("Amanda"));
			AddMoneyToCharacter(PChar, 1000);
			GiveItem2Character(CharacterFromId("Blaze"), "jewelry4");
			AddPartyExp(pchar, 150);
			PlaceCharacter(characterFromID("Amanda"), "goto", "");
			RemovePassenger(pchar, characterFromID("Amanda"));
			CloseQuestHeader("AmandaQuest");
			pchar.quest.AmandaQuest.win_condition = "MainPortugalLineQuest1";
			ChangeCharacterAddressGroup(CharacterFromID("Amanda"), "none", "", "");
			ChangeCharacterAddress(characterFromID("PortugalQuestSoldier"), "Conceicao_port", "goto8");
			break;

		case "MainPortugalLineQuest1":
			LAi_SetActorType(characterFromID("PortugalQuestSoldier"));
			LAi_ActorDialog(characterFromID("PortugalQuestSoldier"), PChar, "", 3.0, 2.0);

			break;

		case "GoToPurtuguezeTownhall":
			pchar.quest.portuguese_line = "started";
			ChangeCharacterAddressGroup(CharacterFromID("PortugalQuestSoldier"), "none", "", "");
			DoQuestReloadToLocation("Conceicao_townhall", "goto", "goto7", "TalkWithBaretto");
			break;

		case "TalkWithBaretto":
			LAi_SetActorType(CharacterFromID("Jacinto Arcibaldo Barreto"));
			LAi_ActorSetSitMode(CharacterFromID("Jacinto Arcibaldo Barreto"));
			LAi_ActorDialog(characterFromID("Jacinto Arcibaldo Barreto"), PChar, "acrcibaldo_back_to_hub_type_again228", 3.0, 1.0);
			break;

		case "acrcibaldo_back_to_hub_type_again228":
			Locations[FindLocation("Conceicao_townhall")].reload.l1.disable = 0;
			LAi_SetHuberType(characterfromID("Jacinto Arcibaldo Barreto"));
			break;

		case "StartPortugalLine":
			pchar.quest.amanda_status = "goSearchAmanda";
			pchar.quest.portuguese_line = "goToBankir";
			AddMoneyToCharacter(PChar, 1000);
			break;

		case "StartPortugalLineWait":
			pchar.quest.portuguese_line = "wait";
			break;

		case "GoToGreenfordBankir":
			pchar.quest.portuguese_line = "searchInformation";
			break;

		case "FerraraWasKilled":
			pchar.quest.portuguese_line = "DeadFerrara";
			pchar.quest.ferrara_status = "DeadStatus";
			break;

		case "FerraraWasKilledInfo":
			pchar.quest.portuguese_line = "DeadFerraraInfo";
			break;

		case "TalkWithBarreto2":
			pchar.quest.portuguese_line = "StartSearchKiller";
			break;

		case "AmandaInDouPirCamp":
			pchar.quest.amanda_status = "amandaInPirCamp";
			break;

		case "AmandaInDouPirCamp2":
			pchar.quest.amanda_status = "goToVanStein";
			ChangeCharacterAddress(characterFromID("VanStein"), "Douwesen_Pirate_HouseInsideR2", "goto2");
			break;

		case "StolenAmanda":
			pchar.quest.amanda_status = "searchAmandaInQC";
			ChangeCharacterAddressGroup(characterFromID("black_raven"), "QC_Tavern", "sit", "sit7");
			ChangeCharacterAddress(characterFromID("blackRavens_pirate"), "QC_Tavern", "goto2");
			ChangeCharacterAddress(characterFromID("blackRavens_pirate2"), "QC_Tavern", "goto4");
			break;

		case "talkWithRaven":
			pchar.quest.amanda_status = "searchFraCorsair";
			break;

		case "disgruntledRaven":
			pchar.quest.amanda_status = "fightWithRavenPirates";
			LAi_LocationFightDisable(&Locations[FindLocation("QC_Tavern")], false);
			LAi_SetActorType(CharacterFromID("blackRavens_pirate"));
			LAi_SetActorType(CharacterFromID("blackRavens_pirate2"));
			LAi_ActorAttack(CharacterFromID("blackRavens_pirate"), PChar, "");
			LAi_ActorAttack(CharacterFromID("blackRavens_pirate2"), PChar, "");
			LAi_SetActorType(pchar);
			LAi_ActorAttack(pchar, characterFromID("blackRavens_pirate"), "");
			LAi_SetPlayerType(PChar);
			ChangeCharacterAddress(characterFromID("VanStein"), "none", "");
			break;

		case "goToFaussurier":
			LAi_LocationFightDisable(&Locations[FindLocation("QC_Tavern")], true);
			pchar.quest.amanda_status = "searchFaussurier";
			ChangeCharacterAddressGroup(characterFromID("black_raven"), "QC_Tavern", "", "");
			ChangeCharacterAddressGroup(characterFromID("blackRavens_pirate"), "QC_Tavern", "", "");
			ChangeCharacterAddressGroup(characterFromID("blackRavens_pirate2"), "QC_Tavern", "", "");
			SetOfficersIndex(Pchar, -1, GetCharacterIndex("black_raven"));
			LAi_SetOfficerType(characterFromID("black_raven"));
			Pchar.quest.amanda_status.win_condition.l1 = "location";
			Pchar.quest.amanda_status.win_condition.l1.character = "black_raven";;
			Pchar.quest.amanda_status.win_condition.l1.location = "Falaise_de_fleur_port_01";
			PChar.quest.amanda_status.win_condition = "blackRavenInFF";
			break;

		case "blackRavenInFF":
			LAi_SetActorType(characterFromID("black_raven"));
			LAi_ActorDialog(characterFromID("black_raven"), PChar, "", 3.0, 2.0);
			pchar.quest.foussur3 = "searchFaussurierInTavernFF234";
			break;

		case "goToFaussurierInFFTavern":
			pchar.quest.amanda_status = "searchFaussurierInTavernFF2";
			LAi_SetOfficerType(characterFromID("black_raven"));
			Pchar.quest.amanda_status.win_condition.l1 = "location";
			Pchar.quest.amanda_status.win_condition.l1.character = "black_raven";;
			Pchar.quest.amanda_status.win_condition.l1.location = "Falaise_de_fleur_tavern";
			PChar.quest.amanda_status.win_condition = "blackRavenInFF2";
			break;

		case "blackRavenInFF2":
			LAi_SetActorType(characterFromID("black_raven"));
			LAi_ActorDialog(characterFromID("black_raven"), PChar, "", 3.0, 2.0);
			pchar.quest.foussur3 = "searchFaussurierInTavernFF235";
			break;

		case "meetFaussurier":
			pchar.quest.foussur3 = "availableFaussurier";
			SetCurrentTime(23, 0);
			//DoReloadCharacterToLocation("Falaise_de_fleur_tavern", "Reload", "Reload1");
			DoQuestReloadToLocation("Falaise_de_fleur_tavern", "reload", "reload1", "meetFaussurier2");
			break;

		case "meetFaussurier2":
			LAi_SetActorType(characterFromID("black_raven"));
			LAi_ActorDialog(characterFromID("black_raven"), PChar, "", 3.0, 2.0);
			break;

		case "meetFaussurier3":
			LAi_SetOfficerType(characterFromID("black_raven"));
			pchar.quest.foussur3 = "availableFaussurier";
			ChangeCharacterAddressGroup(characterFromID("Faussurier"), "Falaise_de_fleur_port_02", "goto", "goto4");
			
			//LAi_SetGuardianType("Faussurier");
			LAi_SetActorType(characterFromID("Faussurier"));
			LAi_ActorDialog(characterFromID("Faussurier"), PChar, "", 5.0, 1.0);
			break;

		case "LessonForBen":
			pchar.quest.foussur3 = "taskFromFaussurier";
			RemoveOfficersIndex(pchar, GetCharacterIndex("black_raven"));
			RemovePassenger(pchar, characterFromID("black_raven"));
			LAi_SetActorType(characterFromID("Faussurier"));
			LAi_ActorGoToLocator(CharacterFromID("Faussurier"), "goto", "soldier_02", "FaussurierGoAway", 9.0);
			ChangeCharacterAddress(characterFromID("BenWales"), "Redmond_HouseInsideR2", "goto2");
			break;

		case "FaussurierGoAway":
			ChangeCharacterAddress(characterFromID("Faussurier"), "", "");
			break;

		case "ShipOfBenWales":
			//AddMoneyToCharacter(PChar, 150000);
			
			//pchar.quest.ShipOfBenWales_status = "yachtInRedAquatory";
			Group_AddCharacter("CadwallonG", "YachtCadwallonCaptain");
			Group_SetGroupCommander("CadwallonG", "YachtCadwallonCaptain");
			Group_SetTaskAttack("CadwallonG", PLAYER_GROUP);
			Group_SetAddress("CadwallonG", "Redmond", "Quest_ships", "Quest_ship_12");

			pchar.quest.ShipOfBenWales_status.win_condition.l1 = "Character_Capture";
			pchar.quest.ShipOfBenWales_status.win_condition.l1.character = "YachtCadwallonCaptain";
			pchar.quest.ShipOfBenWales_status.win_condition = "Capture_Cadwallon";

			pchar.quest.ShipOfBenWales_status.win_condition.l1 = "NPC_Death";
			pchar.quest.ShipOfBenWales_status.win_condition.l1.character = "YachtCadwallonCaptain";
			pchar.quest.ShipOfBenWales_status.win_condition = "Sink_Cadwallon";
			break;

		case "Sink_Cadwallon":
			AddQuestRecord("PortuguezeLine", "14");
			pchar.quest.ShipOfBenWales_status = "Sink_Cadwallon";
			pchar.quest.IsBenWalesLostHisShip = "Yes";
			Group_SetAddress("CadwallonG", "", "", "");
			break;

		case "Capture_Cadwallon":
			pchar.quest.ShipOfBenWales_status = "Capture_Cadwallon";
			pchar.quest.IsBenWalesLostHisShip = "Yes";
			AddQuestRecord("PortuguezeLine", "15");
			Group_SetAddress("CadwallonG", "", "", "");
			break;

		case "DisgruntledBenWales":
			
			ChangeCharacterAddress(characterFromID("BenWalesGuard"), "Redmond_HouseInsideR2", "goto2");
			
			pchar.quest.IsBenWalesLostHisShip = "Next";
			pchar.quest.ShipOfBenWales_status = "revenege";
			DoReloadCharacterToLocation("Redmond_HouseInsideR2", "reload", "reload1");
			LAi_SetActorType(CharacterFromID("BenWalesGuard"));
			LAi_ActorAttack(CharacterFromID("BenWalesGuard"), PChar, "");
			LAi_SetActorType(pchar);
			LAi_ActorAttack(pchar, characterFromID("BenWalesGuard"), "");
			LAi_SetPlayerType(PChar);
			break;
	}
}