	...
	switch (iAbordageMode)
	{
		...
		case SHIP_ABORDAGE:
			// calc & apply abordage damage for ships
				...
				AbordageStike = fHP * (1.05 - stf(Characters[GetMainCharacterIndex()].TmpSkill.Sneak))
				Ship_ApplyHullHitpoints(&Characters[GetMainCharacterIndex()], AbordageStike/1.5, KILL_BY_TOUCH, -1);
...
void Sea_AbordageLoad(int _iAbordageMode, bool _bMCAbordageInitiator)
{
	...
	{
		Log_SetStringToLog("Повторный абордаж судна невозможен.");
