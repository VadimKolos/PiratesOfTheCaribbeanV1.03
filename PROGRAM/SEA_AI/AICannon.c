...
float Cannon_GetRechargeTime()
{
	...
	ref refBaseShip = GetShipByType(sti(aCharacter.ship.Type));
	float cQ = sti(aCharacter.Ship.Crew.Quantity);
	float cMC = sti(refBaseShip.MaxCrew);
	float cCM = sti(aCharacter.Ship.Crew.Morale);

	fReloadTime = fReloadTime * (1 + (1 - cQ / cMC));
	fReloadTime = fReloadTime * (1 + (1 - cCM / 45) / 5);

	//fMultiply = fMultiply * Bring2RangeNoCheck(4.0, 1.0, 0.0, 1.0, stf(aCharacter.Ship.Crew.MinRatio));

	if (!bArcadeCannons) { fMultiply = fMultiply * 12.0; }
