...
void Fantom_SetGoods(ref rFantom, string sFantomType)
{
	...
	switch (sFantomType)
	{
		...
		break;
		case "trade":
			iMultiply = 15 * (rand(iShipClass) + 1);
...
void Fantom_SetPerks(ref rFantom, string sFantomType)
{
	int iFantomRank = sti(rFantom.Rank);
	DeleteAttribute(rFantom, "Perks.list");
	rFantom.Perks.list = "";
	aref arFantomPerks;
	makearef(arFantomPerks, rFantom.Perks.list);
	switch (sFantomType)
	{
		case "war":
			if (rand(100) < 20 * iFantomRank)	arFantomPerks.FastReload = true;
			if (rand(100) < 20 * iFantomRank)	arFantomPerks.HullDamageUp = true;
			if (rand(100) < 20 * iFantomRank)	arFantomPerks.CrewDamageUp = true;
			if (rand(100) < 20 * iFantomRank)	arFantomPerks.SailsDamageUp = true;
			if (rand(100) < 20 * iFantomRank)	arFantomPerks.CriticalShoot = true;
			if (rand(100) < 20 * iFantomRank)	arFantomPerks.LongRangeShoot = true;
			if (rand(100) < 10 * iFantomRank)	arFantomPerks.CannonProfessional = true;
			if (rand(100) < 20 * iFantomRank)	arFantomPerks.BasicBattleState = true;
			if (rand(100) < 15 * iFantomRank)	arFantomPerks.AdvancedBattleState = true;
			if (rand(100) < 10 * iFantomRank)	arFantomPerks.ShipDefenseProfessional = true;
			if (rand(100) < 20 * iFantomRank)	arFantomPerks.ShipSpeedUp = true;
			if (rand(100) < 20 * iFantomRank)	arFantomPerks.ShipTurnRateUp = true;
			if (rand(100) < 10 * iFantomRank)	arFantomPerks.SailingProfessional = true;	
			if (rand(100) < 20 * iFantomRank)	arFantomPerks.LightRepair = true;
		break;
		case "trade":						
			if (rand(100) < 20 * iFantomRank)	arFantomPerks.SailsDamageUp = true;
			if (rand(100) < 20 * iFantomRank)	arFantomPerks.BasicBattleState = true;
