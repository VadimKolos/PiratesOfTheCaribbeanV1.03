...
float Ship_GetBortFireDelta()
{
	...
	if (bArcadeCannons && aCharacter.id == characters[GetMainCharacterIndex()].id)
	{
		fAccuracy  = fAccuracy - 0.2;
	}
	if (aCharacter.id !=characters[GetMainCharacterIndex()].id)
	{
		fAccuracy  = fAccuracy - 0.2;
		if (fAccuracy<0.1)
		{
...
void Ship_SetTaskRunaway(int iTaskPriority, int iCharacterIndex)
{
	...
	//characters[iCharacterIndex].task = "run";
}
...
void Ship_SetTaskAttack(int iTaskPriority, int iCharacterIndex, int iCharacterIndexVictim)
{
	...
	//characters[iCharacterIndex].task = "";
}
...
void Ship_CheckSituation()
{
	...
	//Trace("Right = " + fRightChargeRatio + " , left = " + fLeftChargeRatio);
	if (fRightChargeRatio <= 0.2 || fLeftChargeRatio <= 0.2)
	{
...
int Ship_FindOtherBallType(ref rCharacter, float fMinEnemyDistance, bool bBalls, bool bBombs, bool bGrapes, bool bKnippels)
{
	...
	float range = rCannon.FireRange;
	int enemy = rCharacter.SeaAI.task.target;
	ref rBaseShip = GetShipByType(GetCharacterShipType(rCharacter));
	
	if (rCharacter.SeaAI.Task == AITASK_RUNAWAY && bKnippels)
	{
		if (sti(characters[enemy].ship.sails) > 20)
		{
	...
	{
		if (sti(characters[enemy].ship.sp) > 20)
		{
			return GOOD_KNIPPELS;
		}
		else
	...
	if ( (fMinEnemyDistance > (range * 0.5)) && bBalls) return GOOD_BALLS;
	if ( (fMinEnemyDistance > (range * 0.4)) && bBombs) return GOOD_BOMBS;
	if ( (fMinEnemyDistance > (range * 0.2)) && bGrapes) 
	{
		if (sti(characters[enemy].ship.crew.quantity) > rBaseShip.MinCrew)
		{
			return GOOD_GRAPES;
		}
		else
...
void Ship_ApplyCrewHitpoints(ref rOurCharacter, float fCrewHP)
{
	...
	float fMultiplyMode = 1.0;
	if (!bArcadeSailing) { fMultiplyMode = 1.2; }
	float fNewCrewQuantity = stf(rOurCharacter.Ship.Crew.Quantity) - fCrewHP * fMultiply * fMultiplyMode;
...
void Ship_ApplyHullHitpoints(ref rOurCharacter, float fHP, int iKillStatus, int iKillerCharacterIndex)
{
	...
	float fMultiply = 1.0;
	if (!bArcadeSailing) { fMultiply = 1.2; }
				...
				if (bCompanion)
				{
					for (int i=0; i<COMPANION_MAX; i++)
					{
						...
						if (iIndex >= 0)
						{
							...
							if (CheckAttribute(rMainCharacter, "perks.list.AnchorCollector"))
							{
									GiveItem2Character(&characters[0], "Anchor");
							}
						}
					}
				}
...
void Ship_HullHitEvent()
{
	...
	if (!bDead)
	{
		...
		//float fRigDamage = stf(rBall.DamageRig) * fCannonDamageMultiply * AIShip_isPerksUse(rBallCharacter.TmpPerks.SailsDamageUp, 1.0, 1.15);
		...
		//Ship_ApplyRigHitpoints(rOurCharacter, fRigDamage);
	}
...
void Ship_SetFantomData(ref rFantom)
{
	...
	rFantom.Ship.Crew.Quantity = iMaxCrew - rand(iMinCrew);
	rFantom.Ship.HP = iBaseHP;
	if (rand(3) == 2) { rFantom.Ship.HP = iBaseHP - rand(iBaseHP / 4)};
...
void CharacterUpdateShipFromBaseShip(int iCharacterIndex)
{
	...
	if (!CheckAttribute(rCharacter,"Ship.Crew.Quantity")) { rCharacter.Ship.Crew.Quantity = iMaxCrew - rand(iMinCrew); }
		...
		for (i=0; i<iShipsNum; i++)
		{
			...
			if (bEnemyAngleTest)
			{
				if (fAbordageDistance < fEnemyMaxAbordageDistance)
				{
					...
					if (sti(rShipCharacter.nation) == PIRATE) { fRatio = fRatio * 1.6; }
					if (fRatio > 1.2)
					{
...
void Ship_UpdateParameters()
{
	...
	// calculate MaxSpeedZ
		...
		if (!bArcadeSailing)
		{
			if (fWindDotShip < -0.80) arCharShip.MaxSpeedZ = stf(arCharShip.MaxSpeedZ) * (fWindDotShip + 1.0) / 0.2;
		}
	...
	// calculate MaxSpeedY
		...
		if (!bArcadeSailing)
		{
			...
			{
				case 0: fTRFromSailState = 0.15; break;
				case 1: fTRFromSailState = 1.0; break;
				case 2: fTRFromSailState = 0.5; break;
			}

			fTRResult = Bring2Range(0.1, 1.0, 0.00001, 1.0, fTRFromWeight * fTRFromSpeed * fTRFromSkill * fTRFromPeople * fTRFromSailDamage * fTRFromSailState);
		}
		else
		{
			switch (MakeInt(fSailState * 2.0))
			...
			fTRResult = Bring2Range(0.2, 1.0, 0.00001, 1.0, fTRFromWeight * fTRFromSpeed * fTRFromSkill * fTRFromPeople * fTRFromSailDamage * fTRFromSailState);
		}
	...
	// do damage if storm or tornado
		if (bStorm && bSeaActive)
		{
			...
			// rotate impulse
			float fRotate = stf(arCharShip.Impulse.Rotate.y) + (frnd() * 0.02 - 0.01);
			if (fRotate > 0.03) { fRotate = 0.03; }
			if (fRotate < -0.03) { fRotate = -0.03; }
...
void Ship_UpdateTmpSkills(ref rCharacter)
{
	...
	/*if(!bArcadeMode && rCharacter.id == "blaze")
	{
		aTmpSkill.Accuracy = Clampf(stf(aTmpSkill.Accuracy) * 0.9);
		if (aTmpSkill.Accuracy < 0.1) aTmpSkill.Accuracy = 0.1;
	}*/
}
