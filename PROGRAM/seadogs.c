...
void ProcessControls()
{
	...
	switch(ControlName)
	{
		case "WMapPauseSwitch":
				if (!CheckAttribute(&characters[0], "pause"))
				{
					SetTimeScale(0.0);
				...
				{
					SetTimeScale(1.0);	
					DeleteAttribute(&characters[0], "pause");
				}
		break;
		
		case "TimeScalePlus":
			if (ыеа(characters[0].timescale) < 10.0)
			{
				characters[0].timescale = stf(characters[0].timescale) + 0.5;
			}
			SetTimeScale(stf(characters[0].timescale));	
			Log_SetStringToLog("Текущее ускорение времени: " + characters[0].timescale);
		break;
		
		case "TimeScaleMinus":
			if (stf(characters[0].timescale) > 0.0)
			{
				characters[0].timescale = stf(characters[0].timescale) - 0.5;
			}
			SetTimeScale(stf(characters[0].timescale));	
